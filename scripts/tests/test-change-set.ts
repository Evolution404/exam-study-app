...
import type { PracticeAnswer } from "../../src/lib/db/db";
import type { Attempt, BankFolder, BankQuestionMembership, Bank, PracticeRun, Question, ReviewRound } from "../../src/lib/db/types";
...
const emptyProjection = (): ChangeSetProjection => ({ banks: [], bankFolders: [], questions: [], memberships: [], imageAssets: [], attempts: [], attemptStats: [], attemptDailyStats: [], notes: [], practiceRuns: [], practiceRunStats: [], questionGroups: [], reviewRounds: [], reviewRoundProgress: [], tombstones: [] });
let sequence = 0;
...
projection = reduceChangeSet(projection, base);
...
imported = reduceChangeSet(imported, await cs([{ kind: "question.import", bank: bank("import-bank", "导入题库"), questions: [question("shared-question")], memberships: [membership("import-bank", "shared-question")] }]));
imported = reduceChangeSet(imported, await cs([{ kind: "question.import", bank: { ...bank("import-bank", "导入题库（更新）"), questionCount: 2 }, questions: [question("shared-question"), question("new-question")], memberships: [membership("import-bank", "shared-question"), membership("import-bank", "new-question")] }]));
...
projection = reduceChangeSet(projection, await cs([{ kind: "question.split", originalQuestionId: "question-1", clone: cloneQuestion, memberships: [membership("bank-1", "question-2")], deletedMembershipKeys: ["bank-1:question-1"] }]));
const conflictingSplit = await cs([{ kind: "question.split", originalQuestionId: "question-1", clone: cloneQuestion, memberships: [] }]);
await assert.rejects(async () => reduceChangeSet(projection, conflictingSplit), /已存在/);
const blockedDelete = await cs([{ kind: "question.delete", questionId: "question-2" }]);
await assert.rejects(async () => reduceChangeSet(projection, blockedDelete), /cascade/);
projection = reduceChangeSet(projection, await cs([{ kind: "question.delete.cascade", questionId: "question-2" }]));
...
assert.equal(await digestChangeSet({ ...unorderedA, id: "same", localSequence: 99 }), await digestChangeSet({ ...unorderedB, id: "same", localSequence: 99 }), "bulk ordering is canonical");
