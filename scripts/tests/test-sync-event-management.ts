...
import { discardManagedChangeSet } from "../../src/lib/sync/change-set-queue";
...
// change-sets through discardManagedChangeSet (the handler behind the sync
...
async function pendingId(predicate: (record: { id: string; kind: string }) => boolean): Promise<string> {
  const rows = await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).toArray();
...
try {
  // --- Scenario 1: discarding a pending event rebuilds the projection --------
  {
    ...
    await discardManagedChangeSet(questionChangeSetId);

    assert.equal(await studyDb.questions.count(), 0, "删除 pending 事件后投影应重建为不含该题目");
    assert.ok(await studyDb.banks.get(bank.id), "题库 pending 事件应保留");

    await sync(); // pushes only the bank; the discarded question never reaches the mock
    await freshClient("device-b");
    await sync();
    assert.ok(await studyDb.banks.get(bank.id), "新设备应看到题库");
    assert.equal(await studyDb.questions.count(), 0, "被删除的待同步题目不应出现在其他设备");
  ...
  // --- Scenario 2: cascade deletion of dependent events ---------------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("级联事件题库");
    await createQuestion(bank.id, singleChoice("依赖题库的题目", "A", ["对", "错"])); // membership depends on the bank
    ...
    await assert.rejects(() => discardManagedChangeSet(bankChangeSetId), /依赖/, "有依赖时应拒绝直接删除");

    await discardManagedChangeSet(bankChangeSetId, { cascadeDependents: true });
    assert.equal(await studyDb.banks.count(), 0, "级联删除应移除题库");
    assert.equal(await studyDb.questions.count(), 0, "级联删除应一并移除依赖的题目");
    assert.equal((await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).toArray()).length, 0, "依赖事件应一并删除");
  ...
  // --- Scenario 3: locked (committed) events cannot be deleted --------------
  {
    ...
    const committedBankId = (await studyDb.changeSets.where("state").equals("committed").toArray()).find((record) => record.kind === "bank.create")?.id;
    assert.ok(committedBankId, "应存在已提交的题库事件");
    await assert.rejects(() => discardManagedChangeSet(committedBankId!), /已锁定|不存在/, "已锁定事件不可删除");
    void committedId;
  ...
  // --- Scenario 4: post-discard sync is idempotent and consistent -----------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("幂等题库");
    await createQuestion(bank.id, singleChoice("幂等题", "A", ["对", "错"]));
    await discardManagedChangeSet(await pendingId((record) => record.kind === "batch"));
    ...
    await freshClient("device-b");
    await sync();
    assert.equal(await studyDb.questions.count(), 0, "幂等性：新设备仍不应看到被删题目");
