...
async function seedChangeSet(opts: {
  ...
  const changeSet = await createChangeSet({
    ...
    mutations: opts.mutations as Parameters<typeof createChangeSet>[0]["mutations"],
  });
  await studyDb.changeSets.put({ ...changeSet, state: opts.state ?? "pending", ...(opts.claimId ? { claimId: opts.claimId, claimedAt: opts.claimedAt ?? opts.createdAt } : {}) });
  return changeSet;
}
...
test("S1 CAS 重试：head PUT 409 一次 → rebase → 提交成功", async () => {
  ...
  try {
    ...
    await sync();
    const bank = await createBank("CAS重试题库");
    const q1 = await createQuestion(bank.id, singleChoice("CAS题一", "A", ["对", "错"]));
    const q2 = await createQuestion(bank.id, singleChoice("CAS题二", "A", ["对", "错"]));
    const result = await sync();
    assert.equal(result.pushed, 3, "题库 + 两道题都应推送成功（CAS 重试后提交）");
    assert.equal((await listChangeSets(["claimed"])).length, 0, "不应有 record 卡在 claimed");
    assert.equal((await listChangeSets(["pending", "blocked"])).length, 0, "CAS 重试后应全部 committed");
    await freshClient("device-b");
    await sync();
    assert.ok(await studyDb.questions.get(q1.id), "新设备应拉取到 q1");
    assert.ok(await studyDb.questions.get(q2.id), "新设备应拉取到 q2");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S2 CAS 重试耗尽：持续 409 → 抛「并发更新」且 claim 释放", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("重试耗试题库");
    await createQuestion(bank.id, singleChoice("耗试题", "A", ["对", "错"]));
    await assert.rejects(sync(), /远端持续发生并发更新/, "持续 409 应在重试 4 次后抛出清晰错误");
    assert.equal((await listChangeSets(["claimed"])).length, 0, "claim 应已释放（无 claimed 残留）");
    assert.equal((await listChangeSets(["pending"])).length, 2, "本地变更应回到 pending（不丢）");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S3 部分上传：segment PUT 500 → claim 释放 → 重试成功", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("部分上传题库");
    const q1 = await createQuestion(bank.id, singleChoice("部分上传题", "A", ["对", "错"]));
    await assert.rejects(sync(), /GitHub .* failed \(500\)/, "首个 segment 上传失败应抛错");
    assert.equal((await listChangeSets(["claimed"])).length, 0, "claim 应已释放");
    assert.equal((await listChangeSets(["pending"])).length, 2, "pending 应保留");
    const result = await sync();
    assert.equal(result.pushed, 2, "重试后应推送成功");
    await freshClient("device-b");
    await sync();
    assert.ok(await studyDb.questions.get(q1.id), "重试后的数据应完整可达");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S4 网络抖动：flakyFetch 前 2 次 500 → 重试成功、无半安装", async () => {
  ...
  try {
    ...
    await sync();
    const bank = await createBank("抖动题库");
    const q1 = await createQuestion(bank.id, singleChoice("抖动题", "A", ["对", "错"]));
    const result = await sync();
    assert.equal(result.pushed, 2, "恢复后应推送成功");
    assert.ok(await studyDb.questions.get(q1.id), "本地应无半安装");
  } finally {
    await server.close();
    studyDb.close();
  }
...
// === S5. Corrupt blob — flip next checkpoint download → SyncBlobIntegrityError ===
test("S5 损坏 blob：checkpoint 字节翻转 → 完整性错误、本地不变", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("损坏题库");
    await createQuestion(bank.id, singleChoice("损坏题", "A", ["对", "错"]));
    await sync();
    ...
    assert.equal(await studyDb.questions.count(), 0, "本地应无半安装（题目表为空）");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S6 下载失败：checkpoint blob GET 500 → 干净报错、本地不变", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("下载失败题库");
    await createQuestion(bank.id, singleChoice("下载失败题", "A", ["对", "错"]));
    await sync();
    server.armFailBlobGetOnce();
    await freshClient("device-b");
    await assert.rejects(sync(), /GitHub .* failed \(500\)/, "checkpoint 下载失败应抛错");
    assert.equal(await studyDb.questions.count(), 0, "本地应无半安装");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S7 ETag 304：二次同步走 not-modified 快路径", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("ETag题库");
    await createQuestion(bank.id, singleChoice("ETag题", "A", ["对", "错"]));
    await sync();
...
test("S8 恢复守卫（B1）：有 pending 时 restore 应抛错且不丢数据", async () => {
  ...
  try {
    ...
    const bank = await createBank("恢复题库");
    await createQuestion(bank.id, singleChoice("恢复题", "A", ["对", "错"]));
    await assert.rejects(restoreFullHistoryFromGitHub(settings, "qa-token"), /未同步/, "有未同步变更时 restore 应抛错");
    assert.equal((await listChangeSets(["pending"])).length, 2, "pending 不应被静默丢弃");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S9 中断 claim（B2）：digest 不匹配降级 blocked 而非崩溃", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("中断题库");
    const q1 = await createQuestion(bank.id, singleChoice("中断题", "A", ["对", "错"]));
    await sync();
    ...
    const committed = await listChangeSets(["committed"]);
    const questionRecord = committed[0];
    assert.ok(questionRecord, "应有已提交记录");
    await studyDb.changeSets.put({ ...questionRecord, state: "claimed" as const, claimId: "stale-claim", claimedAt: new Date().toISOString(), digest: `${questionRecord.digest}-tampered` });
    await sync();
    const blocked = await listChangeSets(["blocked"]);
    assert.equal(blocked.length, 1, "digest 不匹配的记录应被标 blocked");
    assert.equal((await listChangeSets(["claimed"])).length, 0, "不应残留 claimed");
    assert.ok(await studyDb.questions.get(q1.id), "远端题目应仍可拉取（sync 未崩溃）");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S10 重放顺序韧性（B3）：乱序 pending 重放抛错时 sync 不崩", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("重放题库");
    const q1 = await createQuestion(bank.id, singleChoice("重放题", "A", ["对", "错"]));
    await sync();
    const question = (await studyDb.questions.get(q1.id))!;
    ...
    await freshClient("device-b");
    await sync();
    assert.equal(await studyDb.questions.get(q1.id), undefined, "删除语义应正确传播（q1 已删）");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S11 并发 bootstrap（B4）：第二设备采纳胜者、无 split-brain", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("bootstrap题库");
    await createQuestion(bank.id, singleChoice("bootstrap题", "A", ["对", "错"]));
    await sync();
    // 第二设备有本地 pending，sync 时不应覆盖第一设备的 head。
    await freshClient("device-b");
    const bankB = await createBank("bootstrap题库B");
    const qB = await createQuestion(bankB.id, singleChoice("bootstrap题B", "A", ["对", "错"]));
    const result = await sync();
    assert.equal(result.pushed, 2, "第二设备应正常推送自己的 pending");
    assert.equal(result.pulled, 2, "第二设备应拉取第一设备的数据");
    assert.ok(await studyDb.questions.get(qB.id), "第二设备本地应有自己的题");
    // 第一设备仍在 mock 远端：重新拉取应看到两设备的全部数据。
    await freshClient("device-c");
    await sync();
    assert.ok(await studyDb.questions.get(qB.id), "无 split-brain：第三设备能看到第二设备的题");
  } finally {
    await server.close();
    studyDb.close();
  }
...
test("S12 并发同步互斥（B5）：Promise.all 双 sync 串行、无 claimed 残留", async () => {
  ...
  try {
    await freshClient("device-a");
    await sync();
    const bank = await createBank("互斥题库");
    await createQuestion(bank.id, singleChoice("互斥题", "A", ["对", "错"]));
    await Promise.all([sync(), sync()]);
    assert.equal((await listChangeSets(["claimed"])).length, 0, "双 sync 后不应有 claimed 残留");
    assert.equal((await listChangeSets(["pending", "blocked"])).length, 0, "双 sync 后应全部 committed");
    await freshClient("device-b");
    await sync();
    assert.equal(await studyDb.questions.count(), 1, "数据只应提交一份（无重复）");
  } finally {
    await server.close();
    studyDb.close();
  }
