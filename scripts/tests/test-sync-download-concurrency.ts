...
//   1. 多段拉取时 blob GET 真正并发（≥2 路），且受 SYNC_DOWNLOAD_CONCURRENCY 封顶；
//   2. 重放顺序不受完成顺序影响 —— 变更集仍按 generation/ordinal 到达；
//   3. 进度报告单调不减、报告次数不少于分段数（进度条功能完整）；
//   4. 跨设备数据一致（并发下载不破坏正确性）。

assert.equal(SYNC_DOWNLOAD_CONCURRENCY, 6, "下载并发度应为 6");
...
const parallelBank = await createBank("检查点并行题库");
await createQuestion(parallelBank.id, question("检查点与单个热窗口必须并行"));
await syncWithGitHub(parallelSettings, "qa-token");
...
const bank = await createBank("并发下载题库");
// 多次小同步各产生独立分段（合并阈值 24 段远未触达）。
for (let round = 0; round < 8; round += 1) {
  for (let index = 0; index < 3; index += 1) {
    await createQuestion(bank.id, question(`并发下载第 ${round * 3 + index} 题：` + "弧垂增大时安全距离随之调整。".repeat(30)));
  }
  await syncWithGitHub(settings, "qa-token");
}
...
assert.ok(server.stats.maxConcurrentBlobReads <= SYNC_DOWNLOAD_CONCURRENCY, `并发峰值不得超过 ${SYNC_DOWNLOAD_CONCURRENCY}（实测 ${server.stats.maxConcurrentBlobReads}）`);
...
const stems = (await studyDb.questions.toArray())
...
await syncWithGitHub(settings, "qa-token");
assert.equal(await studyDb.questions.count(), 24, "第三台设备完整拉取（重复验证正确性）");

await server.close();
studyDb.close();
console.log(`sync download concurrency tests passed: 并发峰值 ${observedPeak}（≤${SYNC_DOWNLOAD_CONCURRENCY}）、顺序保真、进度单调（${segmentStepCount} 段级报告）`);