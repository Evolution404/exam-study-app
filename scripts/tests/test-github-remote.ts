...
const fakeFetch: typeof fetch = async (input, init = {}) => {
  ...
  if (url.pathname.startsWith(blobMarker) && method === "GET") {
    ...
    assert.equal(headers.get("Accept"), GITHUB_RAW_MEDIA_TYPE);
    return new Response(stored, { status: 200, headers: { "Content-Type": "application/octet-stream" } });
  }
  return new Response("not found", { status: 404 });
};

const remote = new GitHubRemote({ owner, repo, branch, token, vaultId, apiBaseUrl: "https://fake.github.test", fetch: fakeFetch, retryDelayMs: 0 });
...
const retryRemote = new GitHubRemote({
...
const headRecoveryRemote = new GitHubRemote({
...
const progressRemote = new GitHubRemote({
...
const assetPath = `${SYNC_ASSET_PREFIX}${digest(largeAsset)}.webp`;
...
const mismatchRemote = new GitHubRemote({ owner, repo, token, vaultId: "other-vault", apiBaseUrl: "https://fake.github.test", fetch: fakeFetch, retryDelayMs: 0 });
await assert.rejects(mismatchRemote.putHead(head), /vault identity/);
const tokenErrorRemote = new GitHubRemote({ owner, repo, token, vaultId, apiBaseUrl: "https://fake.github.test", fetch: async () => new Response(`server says ${token}`, { status: 500 }), retryDelayMs: 0 });
