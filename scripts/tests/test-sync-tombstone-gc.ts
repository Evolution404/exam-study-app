...
import { createBank, createQuestion, studyDb, deleteQuestions, resetDatabase } from "../../src/lib/db/db";
import {
  SYNC_DEVICE_RETIRE_DAYS,
  reclaimableTombstones,
  syncWithGitHub,
} from "../../src/lib/sync/github-sync-engine";
...
function tombstone(deviceId: string, sequence: number): Tombstone {
...
// --- 1. 判定单元 ------------------------------------------------------------
{
  ...
  // 全确认 → 可回收。
  assert.deepEqual(
    reclaimableTombstones([tomb], { devices: { "device-b": { cursors: { "device-a": 10 }, syncedAt: NOW }, "device-c": { cursors: { "device-a": 99 }, syncedAt: NOW } }, headCursors: { "device-a": 12 }, selfDeviceId: "device-a", now: NOW }),
    { keep: [], dropped: 1 },
    "所有已知设备水位 ≥ 墓碑序号 → 可回收",
  );
  // 任一未确认 → 保留。
  assert.deepEqual(
    reclaimableTombstones([tomb], { devices: { "device-b": { cursors: { "device-a": 10 }, syncedAt: NOW }, "device-c": { cursors: { "device-a": 9 }, syncedAt: NOW } }, headCursors: { "device-a": 12 }, selfDeviceId: "device-a", now: NOW }).keep,
    [tomb],
    "任一设备水位未达 → 保留",
  );
  // 未上报水位的设备（仅出现在 cursors 键里）→ 保守保留。
  assert.deepEqual(
    reclaimableTombstones([tomb], { devices: { "device-b": { cursors: { "device-a": 10 }, syncedAt: NOW } }, headCursors: { "device-a": 12, "device-x": 5 }, selfDeviceId: "device-a", now: NOW }).keep,
    [tomb],
    "未上报水位的设备视为未确认 → 保留",
  );
  // 失联超期（退役）→ 从判定集合剔除后可回收。
  const stale = new Date(Date.parse(NOW) - (SYNC_DEVICE_RETIRE_DAYS + 1) * 86_400_000).toISOString();
  assert.deepEqual(
    reclaimableTombstones([tomb], { devices: { "device-b": { cursors: { "device-a": 10 }, syncedAt: NOW }, "device-c": { cursors: { "device-a": 1 }, syncedAt: stale } }, headCursors: { "device-a": 12 }, selfDeviceId: "device-a", now: NOW }),
    { keep: [], dropped: 1 },
    "失联设备从判定集合剔除 → 可回收",
  );
  // 90 天内活跃的慢设备仍阻塞。
  assert.deepEqual(
    reclaimableTombstones([tomb], { devices: { "device-c": { cursors: { "device-a": 1 }, syncedAt: new Date(Date.parse(NOW) - (SYNC_DEVICE_RETIRE_DAYS - 1) * 86_400_000).toISOString() } }, headCursors: {}, selfDeviceId: "device-a", now: NOW }).keep,
    [tomb],
    "90 天内活跃设备仍阻塞回收",
  );
}
...
function question(stem: string): Parameters<typeof createQuestion>[1] {
  return {
    type: "单选",
...
async function currentHead() {
  const remote = createGitHubRemote({ owner: settings.owner, repo: settings.repo, token: "t", apiBaseUrl: server.url });
  const read = await remote.readHead();
  assert.ok(read.initialized);
  return read.head;
}

async function remoteCheckpointTombstones(): Promise<Tombstone[]> {
  ...
  const checkpoint = JSON.parse(text) as { state: { tombstones: Tombstone[] } };
...
await sync();
const bank = await createBank("墓碑题库");
// 建足够体量：一题会被 A 删除且 B/C 都已拉过；另一题承载 C 的 pending 编辑。
for (let index = 0; index < 4; index += 1) await createQuestion(bank.id, question(`墓碑测试第 ${index} 题`));
await sync();
...
const allQuestions = await studyDb.questions.toArray();
...
await deleteQuestions([doomed.id]);
assert.equal(await studyDb.tombstones.count(), 2, "已推送的删除应写题目+关系两条墓碑");
await sync();
...
const headBlobShaBefore = (await createGitHubRemote({ owner: settings.owner, repo: settings.repo, token: "t", apiBaseUrl: server.url }).readHead()).cache?.blobSha;
await sync();
const headBlobShaAfter = (await createGitHubRemote({ owner: settings.owner, repo: settings.repo, token: "t", apiBaseUrl: server.url }).readHead()).cache?.blobSha;
...
const { createChangeSet } = await import("../../src/lib/sync/change-set-codec");
const staleEdit = await createChangeSet({
  deviceId: "device-c",
  localSequence: 1,
  createdAt: "2026-08-13T00:00:00.000Z",
  mutations: [{ kind: "question.upsert", question: { ...doomed, tags: ["离线编辑"], updatedAt: "2026-08-13T00:00:00.000Z" } }],
});
await studyDb.changeSets.put({ ...staleEdit, state: "pending" });
...
await sync();
const stems = (await studyDb.questions.toArray()).map((row) => row.content.map((block) => block.type === "text" ? block.text : "").join(""));
...
await sync();
const importBank = await createBank("导入即删题库");
const created: string[] = [];
for (let index = 0; index < 5; index += 1) {
  const join = await createQuestion(importBank.id, question(`导入即删第 ${index} 题`));
  created.push(join.id);
}
// 未同步（创建事件仍 pending）就删除。
await deleteQuestions(created);
assert.equal(await studyDb.tombstones.count(), 0, "导入即删（未推送）应零墓碑");
const pendingAfterOffset = await studyDb.changeSets.where("state").equals("pending").toArray();
...
await sync();
const eStems = (await studyDb.questions.toArray()).map((row) => row.content.map((block) => block.type === "text" ? block.text : "").join(""));
assert.ok(!eStems.some((text) => text.includes("导入即删")), "抵消后远端从未见过这些题");
assert.equal(await studyDb.tombstones.count(), 0, "抵消场景远端零墓碑到达");
