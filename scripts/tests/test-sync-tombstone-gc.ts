...
import { createBankV7, createQuestionV7, dbV7, deleteQuestionsV7, resetV7Database } from "../../src/lib/db/db-v7";
import {
  SYNC_V7_DEVICE_RETIRE_DAYS,
  reclaimableTombstonesV7,
  syncWithGitHub,
} from "../../src/lib/sync/github-sync-v7";
...
function tombstone(deviceId: string, sequence: number): TombstoneV7 {
...
async function remoteCheckpointTombstones(): Promise<TombstoneV7[]> {
  ...
  const checkpoint = JSON.parse(text) as { state: { tombstones: TombstoneV7[] } };
...
await sync();
const bank = await createBankV7("墓碑题库");
// 建足够体量：一题会被 A 删除且 B/C 都已拉过；另一题承载 C 的 pending 编辑。
for (let index = 0; index < 4; index += 1) await createQuestionV7(bank.id, question(`墓碑测试第 ${index} 题`));
await sync();
...
const allQuestions = await dbV7.questions.toArray();
...
await deleteQuestionsV7([doomed.id]);
assert.equal(await dbV7.tombstones.count(), 2, "已推送的删除应写题目+关系两条墓碑");
await sync();
...
await dbV7.changeSets.put({ ...staleEdit, state: "pending" });
...
assert.equal(await dbV7.questions.count(), 3, "被删题不得复活（只剩 3 题）");
...
await sync();
const stems = (await dbV7.questions.toArray()).map((row) => row.content.map((block) => block.type === "text" ? block.text : "").join(""));
...
await sync();
const importBank = await createBankV7("导入即删题库");
const created: string[] = [];
for (let index = 0; index < 5; index += 1) {
  const join = await createQuestionV7(importBank.id, question(`导入即删第 ${index} 题`));
  created.push(join.id);
}
// 未同步（创建事件仍 pending）就删除。
await deleteQuestionsV7(created);
assert.equal(await dbV7.tombstones.count(), 0, "导入即删（未推送）应零墓碑");
const pendingAfterOffset = await dbV7.changeSets.where("state").equals("pending").toArray();
...
await sync();
const eStems = (await dbV7.questions.toArray()).map((row) => row.content.map((block) => block.type === "text" ? block.text : "").join(""));
assert.ok(!eStems.some((text) => text.includes("导入即删")), "抵消后远端从未见过这些题");
assert.equal(await dbV7.tombstones.count(), 0, "抵消场景远端零墓碑到达");
