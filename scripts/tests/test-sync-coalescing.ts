...
function shortChoice(stem: string): Parameters<typeof createQuestion>[1] {
  return {
    type: "单选",
...
function longChoice(stem: string): Parameters<typeof createQuestion>[1] {
  return {
    type: "单选",
    content: [{ id: "stem-0", type: "text", text: stem }],
    options: ["甲", "乙", "丙", "丁"].map((text, index) => [{ id: `opt-${index}`, type: "text", text }]),
...
function bigInlineChoice(index: number): Parameters<typeof createQuestion>[1] {
  return {
    type: "单选",
...
// ---------------------------------------------------------------------------
{
  ...
  const bank = await createBank("反复合并题库");
  await sync();

  const count = 70;
  const traces: Trace[] = [];
  for (let index = 0; index < count; index += 1) {
    await createQuestion(bank.id, shortChoice(`反复合并第 ${index + 1} 题：考点 ${index} 的正确选项是？`));
  ...
  await sync();
  const pulled = await studyDb.questions.count();
...
// ---------------------------------------------------------------------------
{
  ...
  await freshClient("device-a");
  await sync();
  const bank = await createBank("混合合并题库");
  await sync();

  const smallBefore = 15;
  for (let index = 0; index < smallBefore; index += 1) {
    await createQuestion(bank.id, shortChoice(`混合·前置小同步第 ${index + 1} 题`));
    await sync();
  }
  ...
  for (let index = 0; index < bigBatch; index += 1) {
    await createQuestion(bank.id, longChoice(`${LONG_STEM_BODY}（批次 ${index + 1}）`));
  }
  ...
  for (let guard = 0; guard < SEGMENT_THRESHOLD + 4; guard += 1) {
    ...
    await createQuestion(bank.id, shortChoice(`混合·后置小同步第 ${guard + 1} 题`));
  ...
  await freshClient("device-b");
  await sync();
  const pulledMixed = await studyDb.questions.count();
...
// ---------------------------------------------------------------------------
{
  ...
  await freshClient("device-a");
  await sync();
  const bankA = await createBank("多设备合并题库");
  await sync();
  ...
  for (let index = 0; index < aSmall; index += 1) {
    await createQuestion(bankA.id, shortChoice(`多设备·A 第 ${index + 1} 题`));
    if ((await sync()).coalesced) aCoalesced += 1;
  }
  assert.ok(aCoalesced >= 1, `设备 A 应在积累到阈值后完成合并（实际 ${aCoalesced} 次）`);
  const aQuestionCount = await studyDb.questions.count();
  ...
  await freshClient("device-b");
  const bankB = await createBank("设备B本地题库");
  const bLocal = 3;
  for (let index = 0; index < bLocal; index += 1) {
    await createQuestion(bankB.id, shortChoice(`多设备·B 本地第 ${index + 1} 题`));
  }
  ...
  const bQuestionCount = await studyDb.questions.count();
  ...
  await freshClient("device-a");
  await sync();
  const aResynced = await studyDb.questions.count();
...
// ---------------------------------------------------------------------------
{
  ...
  await freshClient("device-a");
  await sync();
  const bank = await createBank("合并后压缩题库");
  await sync();
  ...
  for (let index = 0; index < smallBeforeCompact; index += 1) {
    await createQuestion(bank.id, shortChoice(`压缩·前置小同步第 ${index + 1} 题`));
  ...
  const bigBatch = 55;
  for (let index = 0; index < bigBatch; index += 1) {
    await createQuestion(bank.id, bigInlineChoice(index));
  }
...
// ---------------------------------------------------------------------------
{
  ...
  for (let index = 0; index < smallAfterImport; index += 1) {
    await createQuestion(bank.id, shortChoice(`卸载合并·小同步第 ${index + 1} 题`));
...
// ---------------------------------------------------------------------------
{
  ...
  await freshClient("device-a");
  await sync();
  const bank = await createBank("大段保留题库");
  await sync();

  // One big sync: 11 heavy inline questions pack into a single ~858 KiB segment
  // (≥ SYNC_COALESCE_LEAVE_BYTES = 512 KiB, so it is classified "large").
  const bigBatch = 11;
  for (let index = 0; index < bigBatch; index += 1) {
    await createQuestion(bank.id, bigInlineChoice(index));
  }
  ...
  let smallCount = 0;
  for (let guard = 0; guard < SEGMENT_THRESHOLD + 4; guard += 1) {
    await createQuestion(bank.id, shortChoice(`大段保留·后置小同步第 ${guard + 1} 题`));
    smallCount += 1;
    const trace = await syncAndTrace(guard + 1);
