...
assert.match(practiceIntent, /sessionStorage/, "explicit pause suppression must be scoped to the current page session");
assert.doesNotMatch(practiceIntent, /localStorage/, "practice recovery must not create a second durable session store");
assert.doesNotMatch(practiceIntent, /runId|currentIndex/, "pause suppression must not duplicate run identity or cursor state");
...
assert.match(practiceController, /suppressPracticeAutoResumeForSession\(\)[\s\S]*?setPracticeSession\(null\)[\s\S]*?setView\("home"\)/, "an explicit pause must suppress auto-resume only for the current page session");
