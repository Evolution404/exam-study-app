...
assert.match(dashboardController, /studyDb\.questionProgress\.bulkGet\(questionIds\)/, "首页题库范围进度必须按当前 questionIds 定向读取 questionProgress");
assert.equal((dashboardController.match(/bankQuestionMemberships\.where\("bankId"\)/g) ?? []).length, 1, "首页进度与统计必须共享一次 membership 范围读取");
