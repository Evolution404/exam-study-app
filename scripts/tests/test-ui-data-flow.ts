...
import { isBankEnabled } from "../../src/lib/db/types";
...
assert.match(bank, /listUnfiledQuestions/);
...
const dbQuestion = readFileSync(new URL("../../src/lib/db/db-question-import.ts", import.meta.url), "utf8");
...
assert.match(bank, /deleteBankWithExclusiveQuestions/, "删除题库应允许同步清理只属于该题库的题目");
assert.match(bank, /只删除题库，保留题目/, "删除题库应保留只删题库选项");
assert.match(bank, /删除题库和独占题目/, "删除题库应提供清理独占题目选项");
assert.match(bank, /removeMemberships/, "试题管理应支持批量移出当前题库");
assert.match(bank, /deleteQuestions/, "试题管理与未归档区应支持批量永久删除");
...
assert.match(history, /readDashboardScopedRows/, "练习结果题目详情必须复用统一 scoped history reader");
...
assert.match(history, /readPracticeHistory\(status, visibleLimit\)/, "练习记录必须通过活动时间派生索引分页读取");
...
const scopeProgressStart = dashboardController.indexOf("const scopeProgressQuery = useLiveQuery");
const scopeStatsStart = dashboardController.indexOf("const scopeStatsQuery = useLiveQuery");
assert.ok(scopeProgressStart >= 0 && scopeStatsStart > scopeProgressStart, "首页轻量进度查询必须与重型统计查询拆开，禁止重新绑定到同一个 Promise");
const scopeProgressSource = dashboardController.slice(scopeProgressStart, scopeStatsStart);
assert.match(scopeProgressSource, /studyDb\.bankQuestionMemberships\.where\("bankId"\)\.anyOf\(activeBankIds\)\.toArray\(\)/, "首页进度只读取当前题库 membership questionId");
assert.match(scopeProgressSource, /studyDb\.questionProgress\.bulkGet\(questionIds\)/, "首页题库范围进度必须按当前 questionIds 定向读取 questionProgress");
assert.doesNotMatch(scopeProgressSource, /readDashboardScopedRows|studyDb\.attempts|studyDb\.notes/, "顶部进度数字不得等待 Attempt/Note 等重型区间统计");
assert.match(dashboardController, /const scopeProgressCache = new Map/, "首页进度必须保留最后一次成功结果，返回首页时同步复用");
assert.match(dashboardController, /const scopeProgress = scopeProgressQuery \?\? scopeProgressCache\.get\(scopeProgressCacheKey\)/, "首页进度异步刷新期间必须优先保留同范围缓存值");
assert.doesNotMatch(dashboardController, /scopeProgressQuery\s*\?\?\s*\{\s*completed:\s*0,\s*total:\s*0/, "loading 状态不得再伪装成真实 0/0 统计");
assert.match(dashboardView, /selectedBanks\.length > 0 && scopeProgress &&/, "冷启动尚未得到真实进度时不得展示假的 0/0/0 chips");
...
assert.match(dashboardController, /const statsBaseQuery = useLiveQuery\(async \(\) => \{\s*if \(view !== "home"\) return null;/, "首页全量统计订阅必须在离开 home 后停止，避免做题时因 questionProgress 更新反复全表重算");
assert.match(dashboardController, /\}, \[view\]\);\s*const pendingCountQuery/, "首页统计生命周期必须跟随 view，待同步计数保持独立常驻订阅");
assert.doesNotMatch(dashboardController, /statsBaseQuery[\s\S]{0,500}questionProgress\.toArray\(\)/, "首页今日目标不得为了未使用的 lifetime 字段读取全量 questionProgress");
assert.match(dashboardController, /questionDailyProgress\.where\("date"\)\.equals\(today\)\.toArray\(\)/, "首页常驻统计只应读取今日聚合行");
...
assert.match(search, /updateQuestions\(targets\.map/, "搜索批量收藏应使用单次原子 bulk update");
assert.match(search, /updateQuestions\(selectedQuestions\.map/, "搜索批量加标签应使用单次原子 bulk update");
...
assert.match(group, /updateQuestions\(targets\.map/, "知识整理标签重命名/删除应使用单次原子 bulk update");
