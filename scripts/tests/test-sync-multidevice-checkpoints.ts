...
function choiceQuestion(stem: string, correctIndex = 0): Parameters<typeof createQuestion>[1] {
...
try {
  ...
  // ===== 设备 A：初始化并生成第一个检查点 =====
  await resetDatabase();
  const bank = await createBank("多设备检查点题库");
  const questionIds: string[] = [];
  for (let index = 1; index <= 36; index += 1) {
    const question = await createQuestion(bank.id, choiceQuestion(`设备 A 基础题目 ${index}`));
  ...
  for (const questionId of questionIds) {
    await saveNote(questionId, `${questionId}:${largeNote}`);
  }
  const pendingBeforeCompaction = await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).count();
  ...
  assert.equal(await studyDb.banks.count(), 1, "设备 B 应同步到 1 个题库");
  assert.equal(await studyDb.questions.count(), 36, "设备 B 应同步到 36 道题");
  assert.equal(await studyDb.notes.count(), 36, "设备 B 应同步到 36 条解析");
  const deviceBId = memoryLocalStorage.get("shijuan-study-device-id") ?? "device-b";

  // ===== 设备 B：新增一道题并推送 =====
  const deviceBBank = await studyDb.banks.toCollection().first();
  assert.ok(deviceBBank, "设备 B 应存在题库");
  await createQuestion(deviceBBank.id, choiceQuestion("设备 B 新增题目", 1));
  ...
  assert.equal(await studyDb.questions.count(), 37, "设备 A 最终应有 37 道题");
  assert.equal(await studyDb.notes.count(), 36, "设备 A 的解析数不应被设备 B 覆盖");
  assert.equal(await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).count(), 0, "设备 A 最终应无待同步变更");

  // ===== 设备 C：空库首次同步，应拉取两个检查点之后的最新状态 =====
  await switchDevice();
  await syncWithGitHub(settings, token);
  assert.equal(await studyDb.banks.count(), 1, "设备 C 应同步到 1 个题库");
  assert.equal(await studyDb.questions.count(), 37, "设备 C 应同步到 37 道题");
  assert.equal(await studyDb.notes.count(), 36, "设备 C 应同步到 36 条解析");
  ...
  async function renameBankOnCurrentDevice(name: string): Promise<void> {
    const currentBank = await studyDb.banks.toCollection().first();
    assert.ok(currentBank, "当前设备应存在题库");
    await updateBank(currentBank.id, { name });
  }
  ...
  assert.equal(await studyDb.changeSets.where("state").equals("pending").count(), 1, "设备 A 推送前应有 1 条待同步变更");
  ...
  assert.equal((await studyDb.banks.toCollection().first())?.name, "A 并发题库名", "设备 A 推送后题库名应为本地方案");
  ...
  assert.equal(await studyDb.changeSets.where("state").equals("pending").count(), 1, "设备 B 推送前应有 1 条待同步变更");
  ...
  assert.equal((await studyDb.banks.toCollection().first())?.name, "B 并发题库名", "设备 B 应 rebase 后写回自己的题库名");

  // C：同样恢复自身数据并产生题库改名事件，注入 409 后应 rebase 并推送。
  await switchDevice(deviceCId);
  await syncWithGitHub(settings, token);
  await renameBankOnCurrentDevice("C 并发题库名");
  assert.equal(await studyDb.changeSets.where("state").equals("pending").count(), 1, "设备 C 推送前应有 1 条待同步变更");
  ...
  assert.equal((await studyDb.banks.toCollection().first())?.name, "C 并发题库名", "设备 C 应 rebase 后写回自己的题库名");
  ...
  for (const [label, deviceId] of [["A", deviceAId], ["B", deviceBId], ["C", deviceCId]] as const) {
    ...
    await syncWithGitHub(settings, token);
    const bankName = (await studyDb.banks.toCollection().first())?.name ?? "";
    assert.equal(await studyDb.questions.count(), 37, `设备 ${label} 最终应有 37 道题`);
    assert.equal(await studyDb.notes.count(), 36, `设备 ${label} 解析数应保持 36`);
    assert.equal(await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).count(), 0, `设备 ${label} 最终应无待同步变更`);
