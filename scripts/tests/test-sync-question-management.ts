...
async function snapshotPending(predicate: (record: PendingSnapshot) => boolean): Promise<PendingSnapshot[]> {
  const records = await dbV6.changeSets.where("state").anyOf(["pending", "blocked"]).toArray();
...
async function restorePending(records: PendingSnapshot[]): Promise<void> {
  for (const record of records) await dbV6.changeSets.put({ ...record, state: "pending" as const, claimedAt: undefined, claimId: undefined });
}
...
try {
  ...
  // questions and the trim survives a sync round-trip to a fresh device.
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("练习中删题");
    const q1 = await createQuestionV6(bank.id, singleChoice("练习题一", "A", ["对", "错"]));
    const q2 = await createQuestionV6(bank.id, singleChoice("练习题二", "A", ["对", "错"]));
    const q3 = await createQuestionV6(bank.id, singleChoice("练习题三", "A", ["对", "错"]));
    const run = await createPracticeRunV6({ bankId: bank.id, questionIds: [q1.id, q2.id, q3.id] });
    await recordPracticeAnswerV6({ runId: run.id, questionId: q1.id, selected: "A", correct: true });
    ...
    const trimmed = await dbV6.practiceRuns.get(run.id);
    ...
    await freshClient("device-b");
    await sync();
    const pulled = await dbV6.practiceRuns.get(run.id);
    assert.deepEqual(pulled?.questionIds, [q1.id, q3.id], "新设备看到的练习同样不含被删题");
    assert.equal(await dbV6.questions.get(q2.id), undefined, "新设备不应看到已删题目");
    assert.equal((await dbV6.questions.get(q1.id))?.id, q1.id, "未删题目全局保留");
  ...
  // --- Scenario 9: deleting a question cascades through notes and groups -----
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("关联清理");
    const q = await createQuestionV6(bank.id, singleChoice("带笔记与题组的题", "A", ["对", "错"]));
    await saveNoteV6(q.id, "错因：混淆了弧垂方向");
    const group = await saveQuestionGroupV6({ name: "易错题组", type: "专题", description: "", items: [{ questionId: q.id, note: "" }] });
    assert.equal((await dbV6.notes.get(q.id))?.content, "错因：混淆了弧垂方向");
    assert.ok(await dbV6.questionGroups.get(group.id));

    await deleteQuestionV6(q.id);
    assert.equal(await dbV6.notes.get(q.id), undefined, "个人解析应随题目级联删除");
    assert.equal(await dbV6.questionGroups.get(group.id), undefined, "题组内题目全删后题组应清除");

    await sync();
    await freshClient("device-b");
    await sync();
    assert.equal(await dbV6.notes.get(q.id), undefined, "新设备解析也应清理");
    assert.equal(await dbV6.questionGroups.count(), 0, "新设备不应残留空题组");
  ...
  // --- Scenario 10: a tombstone prevents a deleted question from reviving ----
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("防复活");
    const q = await createQuestionV6(bank.id, singleChoice("将被删除", "A", ["对", "错"]));
    await sync();
    await deleteQuestionV6(q.id);
    await sync();
    assert.ok(await dbV6.tombstones.get(`question:${q.id}`), "删除应写入墓碑");
    ...
    await freshClient("device-b");
    await sync();
    assert.equal(await dbV6.questions.get(q.id), undefined, "墓碑阻止已删题目在同步后复活");
    assert.ok(await dbV6.tombstones.get(`question:${q.id}`), "新设备也应记录墓碑");
  ...
  // still had the question open cannot push an edit that resurrects it.
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("远端已删本地编辑");
    const q = await createQuestionV6(bank.id, singleChoice("将被另一设备删除", "A", ["对", "错"]));
    await sync();
    assert.equal((await dbV6.questions.get(q.id))?.id, q.id, "device-a 应先看到题目");
    ...
    await freshClient("device-a");
    await sync();
    assert.equal(await dbV6.questions.get(q.id), undefined, "device-a 同步后应感知到删除");
    assert.ok(await dbV6.tombstones.get(`question:${q.id}`), "墓碑应随删除到达 device-a");
    ...
    await assert.rejects(() => updateQuestionV6(q.id, { tags: ["本地改"] }), /不存在或已被删除/, "编辑已删题应被拒绝，不产生复活 upsert");
    assert.equal(await dbV6.changeSets.where("state").equals("pending").count(), 0, "被拒的编辑不应入队待同步");

    // A fresh device confirms the tombstone wins globally.
    await freshClient("device-c");
    await sync();
    assert.equal(await dbV6.questions.get(q.id), undefined, "墓碑优先：远端已删题不应复活");
  ...
  // against the tombstone and is BLOCKED rather than reviving the question.
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("离线编辑遇墓碑");
    const q = await createQuestionV6(bank.id, singleChoice("离线编辑的题", "A", ["对", "错"]));
    await sync();
    ...
    await updateQuestionV6(q.id, { content: [{ id: "stem-0", type: "text", text: "离线后的题干" }], tags: ["离线"] });
    const pendingEdit = (await dbV6.changeSets.where("state").equals("pending").toArray()).find((record) => record.mutations.some((mutation) => mutation.kind === "question.upsert"));
    ...
    await resetV6Database();
    await sync();
    await dbV6.changeSets.put({ ...pendingEdit!, state: "pending", claimedAt: undefined, claimId: undefined });
    ...
    await freshClient("device-c");
    await sync();
    assert.equal(await dbV6.questions.get(q.id), undefined, "墓碑优先：离线编辑不应让已删题复活");
  ...
  // --- Scenario 13 (E6): 删题把题组裁空 → 远端 replay 写墓碑，阻止陈旧组编辑复活 ---
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("E6题组裁空");
    const q = await createQuestionV6(bank.id, singleChoice("E6组内唯一题", "A", ["对", "错"]));
    const group = await saveQuestionGroupV6({ name: "E6易错组", type: "专题", description: "", items: [{ questionId: q.id, note: "" }] });
    await sync();

    // device-b 拉取，离线编辑该组（产生 pending questionGroup.saved）
    await freshClient("device-b");
    await sync();
    assert.ok(await dbV6.questionGroups.get(group.id), "device-b 应先拉到题组");
    await saveQuestionGroupV6({ id: group.id, name: "E6易错组-离线改", type: "专题", description: "", items: [{ questionId: q.id, note: "离线备注" }] });
    ...
    await sync();
    await deleteQuestionV6(q.id);
    assert.equal(await dbV6.questionGroups.get(group.id), undefined, "裁空后题组应被删除");
    assert.ok(await dbV6.tombstones.get(`questionGroup:${group.id}`), "本地应写 questionGroup 墓碑");
    await sync();
    ...
    await freshClient("device-c");
    await sync();
    assert.equal(await dbV6.questionGroups.get(group.id), undefined, "墓碑优先：裁空的题组不应被陈旧编辑复活");
  ...
  // --- Scenario 14 (S2.1): A 删题，B 的活动 run 含该题 → B 同步后 run 被裁剪、行保留 ---
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("跨设备活动run裁剪");
    const q1 = await createQuestionV6(bank.id, singleChoice("活动run题一", "A", ["对", "错"]));
    const q2 = await createQuestionV6(bank.id, singleChoice("活动run题二", "A", ["对", "错"]));
    const run = await createPracticeRunV6({ bankId: bank.id, questionIds: [q1.id, q2.id] });
    await sync();
    ...
    await sync();
    await recordPracticeAnswerV6({ runId: run.id, questionId: q1.id, selected: "A", correct: true });
    assert.deepEqual((await dbV6.practiceRuns.get(run.id))?.questionIds, [q1.id, q2.id], "B 拉取后 run 完整");

    // device-a 删 q1 并推送
    await freshClient("device-a");
    await sync();
    await deleteQuestionV6(q1.id);
    await sync();
    ...
    await resetV6Database();
    await sync();
    assert.equal(await dbV6.questions.get(q1.id), undefined, "B 应感知到 q1 删除");
    const bRun = await dbV6.practiceRuns.get(run.id);
  ...
  // --- Scenario 15 (S3.1): 删独占题库 → 独占题随库删、共享题存活、靶向 run 墓碑化 ---
  {
    ...
    await freshClient("device-a");
    await sync();
    const bankX = await createBankV6("独占删库X");
    const bankY = await createBankV6("共享存留Y");
    const qExclusive = await createQuestionV6(bankX.id, singleChoice("仅X独占题", "A", ["对", "错"]));
    const qShared = await createQuestionV6(bankX.id, singleChoice("X与Y共享题", "A", ["对", "错"]));
    await importQuestionBankV6("共享到Y.json", [{ q: "X与Y共享题", a: ["对", "错"], ans: "A" }]); // 同指纹 → 复用 qShared，建立 Y 关系
    const run = await createPracticeRunV6({ bankId: bankX.id, questionIds: [qExclusive.id, qShared.id] });
    await sync();

    await deleteBankWithExclusiveQuestionsV6(bankX.id);
    await sync();

    await freshClient("device-b");
    await sync();
    assert.equal(await dbV6.banks.get(bankX.id), undefined, "X 题库应删除");
    assert.ok(await dbV6.banks.get(bankY.id), "Y 题库应保留");
    assert.equal(await dbV6.questions.get(qExclusive.id), undefined, "X 独占题应随库删除");
    assert.ok(await dbV6.questions.get(qShared.id), "共享题应存活（Y 仍有关系）");
    assert.equal(await dbV6.practiceRuns.get(run.id), undefined, "靶向 X 的 run 应被删除");
    assert.ok(await dbV6.tombstones.get(`bank:${bankX.id}`), "应写 bank 墓碑");
    assert.ok(await dbV6.tombstones.get(`practiceRun:${run.id}`), "应写靶向 run 墓碑");
    assert.ok(await dbV6.tombstones.get(`question:${qExclusive.id}`), "应写独占题墓碑");
    console.log("scenario 15 passed: 删独占题库级联（独占题删/共享题存活/靶向run墓碑）（S3.1）");
  }

  // --- Scenario 16 (S3.4): deleteBankV6 总墓碑化靶向 run vs deletePracticeRunV6 条件墓碑 ---
  {
    server.reset();
    await freshClient("device-a");
    await sync();
    const bank = await createBankV6("墓碑策略对比");
    const q = await createQuestionV6(bank.id, singleChoice("墓碑对比题", "A", ["对", "错"]));
    const runNoAnswer = await createPracticeRunV6({ bankId: bank.id, questionIds: [q.id] }); // 无作答
    await sync();
    await deleteBankV6(bank.id);
    await sync();
    assert.equal(await dbV6.practiceRuns.get(runNoAnswer.id), undefined, "靶向 run 应删除");
    assert.ok(await dbV6.tombstones.get(`practiceRun:${runNoAnswer.id}`), "deleteBankV6 即使 run 无作答也总写墓碑");

    // 对照：直接 deletePracticeRunV6 一个无作答 run → 无墓碑（E7 特征化）
    await freshClient("device-b");
    await sync();
    const bank2 = await createBankV6("直接删run对比");
    const q2 = await createQuestionV6(bank2.id, singleChoice("直接删run题", "A", ["对", "错"]));
    const directRun = await createPracticeRunV6({ bankId: bank2.id, questionIds: [q2.id] });
    await sync();
    await deletePracticeRunV6(directRun.id);
    assert.equal(await dbV6.practiceRuns.get(directRun.id), undefined, "直接删 run 应删除");
    assert.equal(await dbV6.tombstones.get(`practiceRun:${directRun.id}`), undefined, "deletePracticeRunV6 对无作答 run 不写墓碑（E7）");
