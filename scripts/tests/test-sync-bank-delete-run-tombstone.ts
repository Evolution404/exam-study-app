...
const deleteBank = await createChangeSet({ id: "delete-bank", deviceId: "device-a", localSequence: 1, createdAt: at, mutation: { kind: "bank.delete.cascade", bankId: bank.id, deletedAt: at } });
const afterDelete = reduceChangeSet(base, deleteBank);
const staleRunSave = await createChangeSet({ id: "stale-run", deviceId: "device-b", localSequence: 1, createdAt: at, mutation: { kind: "practice.run.saved", run: { ...run, deviceId: "device-b" } } });
