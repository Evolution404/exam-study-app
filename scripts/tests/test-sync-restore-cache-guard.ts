...
try {
  ...
  await resetDatabase();
  const syncedBank = await createBank("已同步题库");
  await createQuestion(syncedBank.id, {
    type: "单选",
  ...
  const pendingAfter = await studyDb.changeSets.where("state").anyOf(["pending", "blocked"]).count();
  assert.equal(pendingAfter, 0, "本地恢复应清空待同步事件（丢弃是功能语义）");

  const bankNames = (await studyDb.banks.toArray()).map((bank) => bank.name);
