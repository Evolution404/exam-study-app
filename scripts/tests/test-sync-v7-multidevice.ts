...
for (let count = 1; count <= 100; count += 1) {
  const decision = planSyncV7Compaction({ head: { formatVersion: 9, vaultId: "vault", generatedAt: at, generation: count, metadata: { vaultId: "vault" }, checkpoint: { path: `sync/v9/checkpoints/${"a".repeat(64)}.json`, blobSha: "b".repeat(40), sha256: "a".repeat(64), size: 100 }, segments: [], cursors: {} }, hotBytes: count * 512 });
