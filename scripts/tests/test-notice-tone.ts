...
const SYNC_ERRORS: string[] = [
  "sync head vault identity does not match this remote",
  "sync append vault identity mismatch",
  "sync publication vault identity mismatch",
  "sync segment vault identity mismatch",
  "sync segment vault identity does not match head",
  ...
  "GitHub read sync head failed (404)",
  "GitHub put immutable sync/v9/objects/abc.json failed (422)",
  "sync segments exceed the aggregate hot-window byte limit; compact explicitly first",
  "sync segments exceed the bounded index limit",
  "sync replay contains duplicate generation/ordinal",
  "sync segment generation mismatch",
  "sync segment ordinal mismatch",
  "immutable sync file content differs at sync/v9/objects/abc.json",
  "sync blob size mismatch: expected 1024, received 2048",
  "sync blob sha256 mismatch: expected aaaa, received bbbb",
  "object ref path digest must equal sha256",
  "invalid sync segment envelope",
  "invalid sync segment JSON",
  "远程检查点不是有效 JSON。",
  "正则表达式格式不正确，请检查后重试",
  "题库导入失败",
  "本地缓存恢复失败",
