...
import { readDashboardScopedRowsV7 } from "../../src/app/shell/dashboard-read-data";
...
dbV7.attempts.hook("reading").unsubscribe(attemptHook);
assert.equal(scoped.attempts.length, 21, "指定题目 lifetime 统计必须完整保留该题历史");
assert.equal(attemptReads, 21, "指定题库统计不得 materialize 其他题目的 attempts");
