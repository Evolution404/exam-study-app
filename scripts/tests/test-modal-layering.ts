...
assert.match(styles, /\.simple-dialog-backdrop\s*\{[^}]*z-index:\s*130/, "dialogs must layer above the mobile tab bar");
assert.match(styles, /\.editor-backdrop\s*\{[^}]*position:\s*fixed;[^}]*inset:\s*0;[^}]*z-index:\s*120/, "nested question editor must layer above detail drawers");
assert.match(styles, /@media\s*\(max-width:\s*760px\)[\s\S]*?\.restore-card-actions\s*\{[^}]*grid-column:\s*1\/-1;[^}]*width:\s*100%;[^}]*display:\s*grid;[^}]*grid-template-columns:\s*repeat\(2,minmax\(0,1fr\)\)/, "mobile restore actions must occupy their own full-width row");
assert.ok(confirmDialog.includes("dialog-progress"), "busy confirmation dialogs must render operation progress");
assert.ok(studyApp.includes("top-sync-progress") || read("src/app/shell/topbar.tsx").includes("top-sync-progress"), "top-bar sync must expose page-level progress");
