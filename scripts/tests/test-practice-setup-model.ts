...
assert.match(readSource, /dbV7\.questionProgress\.bulkGet\(ids\)/, "Practice Setup questionProgress 必须按 questionId 主键定向读取");
...
assert.doesNotMatch(readSource, /dbV7\.(?:questionProgress|reviewRoundProgress|attempts)\.toArray\(\)/, "Practice Setup read-model 不得回退历史全表扫描");
...
assert.doesNotMatch(sessionControllerSource, /dbV7\.questionProgress\.toArray\(\)[\s\S]*dbV7\.reviewRoundProgress\.toArray\(\)[\s\S]*dbV7\.attempts\.toArray\(\)/, "开始练习不得恢复全量历史三表扫描");
