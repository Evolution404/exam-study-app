...
const deleteEvent = await createChangeSet({ id: "delete-attempt", deviceId: "device-a", localSequence: 1, createdAt: at, mutation: { kind: "attempt.delete", attemptId: attempt.id, deletedAt: at } });
const afterDelete = reduceChangeSet(base, deleteEvent);
...
const staleCreate = await createChangeSet({ id: "stale-attempt", deviceId: "device-b", localSequence: 1, createdAt: at, mutation: { kind: "attempt.create", attempt: { ...attempt, deviceId: "device-b", elapsedMs: 900 } } });
assert.throws(() => reduceChangeSet(afterDelete, staleCreate), /已被删除|conflict|墓碑|不存在/, "陈旧 attempt.create 不应复活已删除的作答记录");
