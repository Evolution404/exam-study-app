...
async function runDesktop(page, mockServer) {
  ...
  await clickTextButton(page, "立即同步");
  await expectNotice(page, /v9 同步完成/, "real sync success notice");
  ...
  assert.ok(mockServer.contentPaths().includes("sync/v9/head.json"), "mock backend must hold the v8 head after a real sync");
  assert.ok(mockServer.contentPaths().some((path) => path.startsWith("sync/v9/checkpoints/")), "mock backend must hold the initial checkpoint");
  ...
  await clickTextButton(page, "立即同步");
  await expectNotice(page, /v9 同步完成/, "idempotent second sync");
}
...
async function runMobile(page, mockServer) {
  ...
  await realFields.nth(4).fill(mockServer.url);
  await clickTextButton(page, "立即同步");
  await expectNotice(page, /v9 同步完成/, "second-device real sync success");
...
async function runManagementQA(page, mockServer) {
  ...
  assert.match(syncToast, /v9 同步完成/, "management events should sync successfully");
  ...
  await expectNotice(page, /v9 同步完成/, "drawer quick sync notice");
  await page.waitForTimeout(600);
...
async function runSyncRefreshQA(page, mockServer) {
  ...
  assert.ok(quickSyncBox, "quick-sync button must have a pointer target");
  ...
  assert.equal(await quickSyncButton.evaluate((button) => button.classList.contains("holding")), true, "dead-zone press must expose the restore-hold state");
  await page.mouse.up();
  await page.waitForTimeout(250);
  assert.equal(mockServer.stats.totalRequests, requestsBeforeCancelledHold, "releasing in the hold dead zone must not send any sync request");
  assert.equal(await quickSyncButton.evaluate((button) => button.classList.contains("holding")), false, "cancelled hold must clear its visual state");
