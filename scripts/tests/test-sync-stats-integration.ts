...
async function answer(runId: string, questionId: string, correct: boolean, selected: string | string[], createdAt?: string, elapsedMs = 120): Promise<void> {
  await recordPracticeAnswer({ runId, questionId, selected, correct, elapsedMs, ...(createdAt ? { createdAt } : {}) });
}
...
try {
  // --- Scenario 1: correct / wrong / give-up stats round-trip --------------
  {
    ...
    await sync();
    const bank = await createBank("统计基础题库");
    const q1 = await createQuestion(bank.id, singleChoice("一加一等于几？", "B", ["1", "2", "3"]));
    const q2 = await createQuestion(bank.id, singleChoice("地球是圆的吗？", "A", ["是", "否"]));
    const q3 = await createQuestion(bank.id, singleChoice("放弃的题目", "A", ["对", "错"]));
    ...
    await pushThenVerify("device-b", async () => {
      const stats = await studyDb.questionProgress.toArray();
      ...
      assert.equal(await studyDb.attempts.count(), 3, "三条作答记录应完整同步");
  ...
  // --- Scenario 2: repeated answers accumulate streaks across dates ---------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("连续作答题库");
    const q = await createQuestion(bank.id, singleChoice("连续练习", "A", ["对", "错"]));
    ...
    await pushThenVerify("device-b", async () => {
      const stat = await studyDb.questionProgress.get(q.id);
      ...
      const daily = await studyDb.questionDailyProgress.where("questionId").equals(q.id).toArray();
  ...
  // --- Scenario 3: two devices answer concurrently and merge ---------------
  {
    ...
    const q = await createQuestion(bank.id, singleChoice("并发题", "A", ["对", "错"]));
    const runA = await createPracticeRun({ bankId: bank.id, questionIds: [q.id] });
    await answer(runA.id, q.id, true, "A", DAY_1);
    await sync();

    await freshClient("device-b");
    await sync();
    const runB = await createPracticeRun({ bankId: bank.id, questionIds: [q.id] });
  ...
  // --- Scenario 4: practice-run status transitions + run stats -------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("练习统计题库");
    const q = await createQuestion(bank.id, singleChoice("练习状态题", "A", ["对", "错"]));
    const run = await createPracticeRun({ bankId: bank.id, questionIds: [q.id] });
    await answer(run.id, q.id, true, "A", DAY_1);
    await setPracticeRunStatus(run.id, "completed");

    await pushThenVerify("device-b", async () => {
      const pulledRun = await getPracticeRun(run.id);
      assert.equal(pulledRun?.status, "completed", "练习完成状态应同步");
      assert.equal(Object.keys(pulledRun!.answers).length, 1, "作答应随练习同步");
      const runStats = await studyDb.bankPracticeStats.get(bank.id);
      assert.ok(runStats);
      assert.deepEqual(
  ...
  // --- Scenario 5: deleting a run keeps global attempt stats ----------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("删除练习题库");
    const q = await createQuestion(bank.id, singleChoice("删除练习题", "A", ["对", "错"]));
    const run = await createPracticeRun({ bankId: bank.id, questionIds: [q.id] });
    await answer(run.id, q.id, true, "A");
    await deletePracticeRun(run.id);

    await pushThenVerify("device-b", async () => {
      assert.equal(await studyDb.practiceRuns.get(run.id), undefined, "练习记录应被删除");
      const stat = await studyDb.questionProgress.get(q.id);
      assert.ok(stat, "删除练习不应回退全局作答统计");
      assert.equal(stat.total, 1);
      assert.equal(await studyDb.attempts.count(), 1, "作答记录保留为全局学习历史");
    });
  ...
  // --- Scenario 6: cascade bank delete clears stats --------------------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("级联删除统计题库");
    const q = await createQuestion(bank.id, singleChoice("将被级联删除", "A", ["对", "错"]));
    const run = await createPracticeRun({ bankId: bank.id, questionIds: [q.id] });
    await answer(run.id, q.id, true, "A");
    await deleteBankWithExclusiveQuestions(bank.id);

    await pushThenVerify("device-b", async () => {
      assert.equal(await studyDb.questions.get(q.id), undefined, "级联删除应移除题目");
      assert.equal(await studyDb.attempts.count(), 0, "级联删除应移除作答");
      assert.equal(await studyDb.questionProgress.get(q.id), undefined, "级联删除应移除全局统计");
      assert.equal(await studyDb.questionDailyProgress.where("questionId").equals(q.id).count(), 0, "级联删除应移除每日统计");
    });
  ...
  // --- Scenario 7: review-round progress syncs -------------------------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("复习轮次题库");
    const q1 = await createQuestion(bank.id, singleChoice("复习题 1", "A", ["对", "错"]));
    const q2 = await createQuestion(bank.id, singleChoice("复习题 2", "B", ["对", "错"]));
    const round = await createReviewRound({ name: "错题复习", bankIds: [bank.id] });
    const run = await createPracticeRun({ bankId: bank.id, questionIds: [q1.id, q2.id], reviewRoundId: round.id });
    await answer(run.id, q1.id, true, "A");
    await answer(run.id, q2.id, false, "A");
    await completeReviewRound(round.id);

    await pushThenVerify("device-b", async () => {
      const pulledRound = await studyDb.reviewRounds.get(round.id);
      assert.equal(pulledRound?.status, "completed", "轮次完成状态应同步");
      const progress = await studyDb.reviewRoundProgress.where("roundId").equals(round.id).toArray();
  ...
  // --- Scenario 8: groups / notes / folders / favourites ---------------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("杂项操作题库");
    const folder = await saveBankFolder({ name: "同步文件夹", description: "验证" });
    const q1 = await createQuestion(bank.id, singleChoice("收藏题", "A", ["对", "错"]));
    const q2 = await createQuestion(bank.id, singleChoice("解析题", "A", ["对", "错"]));
    await saveNote(q2.id, "这是一条个人解析。");
    await saveQuestionGroup({ name: "题组 A", type: "错题", description: "同步验证", items: [{ questionId: q1.id, note: "重点" }, { questionId: q2.id, note: "" }] });
    await toggleQuestionFavorite(q1.id);
    await saveBankFolder({ id: folder.id, name: "同步文件夹", description: "更新后的描述" });

    await pushThenVerify("device-b", async () => {
      assert.ok(await studyDb.bankFolders.get(folder.id), "文件夹应同步");
      assert.equal((await studyDb.questions.get(q1.id))?.favorite, true, "收藏状态应同步");
      const note = await studyDb.notes.get(q2.id);
      assert.equal(note?.content, "这是一条个人解析。", "解析应同步");
      const group = (await listQuestionGroups())[0];
  ...
  // --- Scenario 9: question split propagates a clone -------------------------
  {
    ...
    await freshClient("device-a");
    await sync();
    const bank = await createBank("拆分题库");
    const original = await createQuestion(bank.id, singleChoice("待拆分题", "A", ["对", "错"]));
    const { clones } = await splitQuestion(original.id, [bank.id]);
    assert.equal(clones.length, 1, "拆分应产生一个 clone");

    await pushThenVerify("device-b", async () => {
      assert.ok(await studyDb.questions.get(original.id), "原题应保留");
      assert.ok(await studyDb.questions.get(clones[0].id), "clone 应同步到新设备");
      assert.equal(await studyDb.questions.count(), 2, "拆分后应有两道题");
    });
