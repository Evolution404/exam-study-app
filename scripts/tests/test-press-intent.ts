...
assert.match(quickSyncController, /if \(document\.visibilityState === "hidden"\) resetQuickSyncPress\(\)/, "quick-sync press is cancelled when the document is hidden");
assert.match(quickSyncController, /window\.addEventListener\("pagehide", cancelOnLifecycle\)/, "quick-sync press is cancelled on pagehide");
assert.match(quickSyncController, /window\.addEventListener\("blur", cancelOnLifecycle\)/, "quick-sync press is cancelled when the window loses focus");
...
assert.match(quickSyncController, /shouldCancelQuickSyncMove\(dx, dy\)/, "the button must use the tested movement classifier");
