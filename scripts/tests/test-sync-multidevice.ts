...
const deviceAImport = await createChangeSet({ id: "a-import", deviceId: "device-a", localSequence: 1, createdAt: at, mutations: [{ kind: "question.import", bank: { ...bank, id: "bank-a", name: "A 导入", deviceId: "device-a" }, questions: [question("question-a", "device-a")], memberships: [{ ...membership("question-a", "device-a"), key: "bank-a:question-a", bankId: "bank-a" }] }] });
const deviceBQuestion = await createChangeSet({ id: "b-question", deviceId: "device-b", localSequence: 1, createdAt: at, mutations: [{ kind: "question.upsert", question: question("question-b", "device-b") }, { kind: "membership.save", membership: membership("question-b", "device-b") }] });
...
const wire = replaySyncSegments([
  { generation: 2, ordinal: 0, path: "sync/v9/segments/000.json", events: [deviceBQuestion] },
  { generation: 1, ordinal: 0, path: "sync/v9/segments/fff.json", events: [deviceAImport] },
]);
...
const editA = await createChangeSet({ id: "edit-a", deviceId: "device-a", localSequence: 2, createdAt: at, mutation: { kind: "bank.update", bank: { ...bank, name: "A 名称", deviceId: "device-a" } } });
const editB = await createChangeSet({ id: "edit-b", deviceId: "device-b", localSequence: 2, createdAt: at, mutation: { kind: "bank.update", bank: { ...bank, name: "B 名称", deviceId: "device-b" } } });
let left = structuredClone(empty);
for (const change of replaySyncSegments([{ generation: 8, ordinal: 0, events: [editB] }, { generation: 7, ordinal: 0, events: [editA] }])) left = reduceChangeSet(left, change);
let right = structuredClone(empty);
for (const change of replaySyncSegments([{ generation: 7, ordinal: 0, events: [editA] }, { generation: 8, ordinal: 0, events: [editB] }])) right = reduceChangeSet(right, change);
...
const deleted = await createChangeSet({ id: "delete", deviceId: "device-a", localSequence: 3, createdAt: at, mutation: { kind: "question.delete", questionId: "question-b", cascade: true, deletedAt: at } });
const staleEdit = await createChangeSet({ id: "stale", deviceId: "device-b", localSequence: 3, createdAt: at, mutation: { kind: "question.upsert", question: { ...question("question-b", "device-b"), answer: "B" } } });
const afterDelete = reduceChangeSet(merged, deleted);
assert.throws(() => reduceChangeSet(afterDelete, staleEdit), /不存在|conflict|deleted/i);
...
for (let count = 1; count <= 100; count += 1) {
  const decision = planSyncCompaction({ head: { formatVersion: 9, vaultId: "vault", generatedAt: at, generation: count, metadata: { vaultId: "vault" }, checkpoint: { path: `sync/v9/checkpoints/${"a".repeat(64)}.json`, blobSha: "b".repeat(40), sha256: "a".repeat(64), size: 100 }, segments: [], cursors: {} }, hotBytes: count * 512 });
