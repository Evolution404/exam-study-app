...
import { SYNC_MAX_EVENT_BYTES, SYNC_OBJECT_PREFIX } from "../../src/lib/sync/sync-head-types";
import { encodeSyncEvent } from "../../src/lib/sync/sync-head-operations";
import { hydrateSyncEvents, offloadedRefOf, offloadSyncEvents, SYNC_INLINE_EVENT_BUDGET } from "../../src/lib/sync/sync-payload";
import type { Bank, Question } from "../../src/lib/db/types";

const at = "2026-08-13T00:00:00.000Z";
const bank: Bank = { id: "bank-1", name: "载荷题库", sortOrder: 0, questionCount: 0, importedAt: at, updatedAt: at, deviceId: "dev-a" };
...
const big = await createChangeSet({ deviceId: "dev-a", localSequence: 1, createdAt: at, mutation: { kind: "question.upsert", question: bigQuestion() } });
const small = await createChangeSet({ deviceId: "dev-a", localSequence: 2, createdAt: at, mutation: { kind: "bank.create", bank } });

// The budget sits below the hard inline ceiling so inline events stay safe.
assert.equal(SYNC_INLINE_EVENT_BUDGET, 128 * 1024);
assert.ok(SYNC_INLINE_EVENT_BUDGET < SYNC_MAX_EVENT_BYTES, "inline budget must stay under the hard event ceiling");

// (1) Without offload, an oversized body cannot be encoded inline — this is the
// error the offload path exists to eliminate.
assert.throws(() => encodeSyncEvent(big), /exceeds 262144/);

// (2) Offload moves the body into one content-addressed immutable object and
// leaves a thin stub.
const offloaded = await offloadSyncEvents([big as unknown as Record<string, unknown>]);
...
assert.equal(ref!.path.slice(SYNC_OBJECT_PREFIX.length, SYNC_OBJECT_PREFIX.length + 64), objectSha, "the path embeds the content digest");
...
assert.doesNotThrow(() => encodeSyncEvent(stub));

// (3) Hydration is lossless: stub → object → full change-set === original.
const store = new Map([[objectFile.path, objectBytes]]);
const hydrated = await hydrateSyncEvents(offloaded.events, (r) => Promise.resolve(store.get(r.path)!));
...
const mixed = await offloadSyncEvents([small, big, small] as unknown as Record<string, unknown>[]);
...
const mixedHydrated = await hydrateSyncEvents(mixed.events, (r) => Promise.resolve(mixedStore.get(r.path)!));
...
await assert.rejects(() => hydrateSyncEvents(offloaded.events, () => Promise.resolve(tampered)), /integrity/, "a mutated body must fail the sha256/size check");

// (6) Idempotency: identical bodies hash to identical paths, so re-publication
// never conflicts and never duplicates.
const reoffloaded = await offloadSyncEvents([big as unknown as Record<string, unknown>]);
...
const tinyResult = await offloadSyncEvents([small as unknown as Record<string, unknown>]);
