...
} from "../../lib/sync/github-v7-remote";
import {
...
import type { SyncHeadV7 } from "../../lib/sync/sync-v7-head";
...
const fakeFetch: typeof fetch = async (input, init = {}) => {
  ...
  const headPath = `/repos/${owner}/${repo}/contents/sync/v7/head.json`;
  if (url.pathname === headPath) {
    ...
    if (method === "PUT") {
      ...
      return json({ content: { path: "sync/v7/head.json", sha: headSha } }, counter === 1 ? 201 : 200, { ETag: headEtag });
    }
  }
