...
const deleteEvent = await createChangeSet({ id: "delete-note", deviceId: "device-a", localSequence: 1, createdAt: at, mutation: { kind: "note.deleted", questionId: question.id, deletedAt: at } });
...
const staleUpsert = await createChangeSet({ id: "stale-note", deviceId: "device-b", localSequence: 1, createdAt: at, mutation: { kind: "note.upserted", note: { ...note, content: "陈旧解析", revision: 2, deviceId: "device-b" } } });
assert.throws(() => reduceChangeSet(afterDelete, staleUpsert), /已被删除|conflict|墓碑|不存在/, "陈旧解析不应复活已删除的解析");
