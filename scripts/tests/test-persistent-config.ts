...
const values = new Map<string, string>([
  ...
  ["study-preferences", JSON.stringify({ themeMode: "dark" })],
...
assert.equal(getPersistentConfigMirror("study-preferences"), JSON.stringify({ themeMode: "dark" }));
assert.equal(getPersistentConfigMirror("shijuan-study-device-id"), "device-persisted");

await persistConfigValue("study-preferences", JSON.stringify({ themeMode: "light" }));
assert.equal(values.get("study-preferences"), JSON.stringify({ themeMode: "light" }));
failSet = true;
