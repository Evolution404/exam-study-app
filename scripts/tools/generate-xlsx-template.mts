...
const questionRows = [
  ...
  questionRow("示例·单选（填好后删）导线的主要作用是什么？", "单选", "基础，示例", "示例解析：导线的作用是传输电能，其余选项为干扰项。", ["A"], ["传输电能", "装饰线路", "储存电能", "测量温度"]),
  questionRow("示例·多选（填好后删）安全巡视应做到哪些？", "多选", "安全，示例", "示例解析：安全巡视应核对线路编号并佩戴防护用品。", ["AB"], ["佩戴防护用品", "核对线路编号", "跨越警戒区域", "跳过危险点记录"]),
  questionRow("示例·判断（填好后删）巡视前应确认天气和现场风险。", "判断", "安全，示例", "示例解析：巡视前应确认天气与现场风险，确保安全。", ["A"], ["正确", "错误"]),
  questionRow("示例·单空计算（填好后删）某量计算结果为【空1】。", "计算", "计算，示例", "示例解析：答案1 对应题干中的【空1】。", ["12.5"]),
  questionRow("示例·多空计算（填好后删）将 R=8Ω 的电阻和容抗 XC=6Ω 的电容器串联，接在频率为 f=50Hz、电压为 U=110V 的正弦交流电源上。电路中的电流为【空1】A，所消耗的有功功率为【空2】W。", "计算", "计算，多空，示例", "示例解析：两个填空分别对应答案1和答案2，每个空独立按误差比例判定。", ["11.0", "968.0"]),
  questionRow("示例·图片单选（填好后删）图中图标对应哪个应用？【图1】", "单选", "图片，示例", "示例解析：图片1 嵌入在本行最后一列，题干中的【图1】决定图片显示位置。", ["A"], ["拾卷", "浏览器", "相册", "日历"], `=DISPIMG("${templateImageId}",1)`),
  questionRow("示例·单空填空（填好后删）输电线路的主要作用是【空1】。", "填空", "填空，示例", "示例解析：同一空允许多个等价答案时，在答案单元格内用 || 分隔。", ["传输电能||输送电能"]),
  questionRow("示例·多空填空（填好后删）三相交流电中，相邻两相电动势的相位差为【空1】°，我国电力系统工频为【空2】Hz。", "填空", "填空，多空，示例", "示例解析：答案1对应空1、答案2对应空2；每个空可分别设置多个等价答案。", ["120", "50||50赫兹"]),
  questionRow("示例·简答（填好后删）简述输电线路巡视前应完成的主要安全准备。", "简答", "简答，安全，示例", "示例解析：简答题由答题者对照参考答案自行判定正确或错误。", ["确认天气和现场风险，核对线路及作业信息，准备并检查必要的安全防护用品和巡视工具。"]),
];
