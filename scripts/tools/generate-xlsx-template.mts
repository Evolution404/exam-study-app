...
function questionRow(
  ...
  return [
    ...
    tags,
    note,
    ...Array.from({ length: answerColumnCount }, (_, index) => answers[index] ?? ""),
    ...optionHeaders.map((_, index) => options[index] ?? ""),
    image,
  ];
}

const questionRows = [
  ["题干", "题型", "标签", "解析", ...answerHeaders, ...optionHeaders, "图片1"],
  questionRow(
    "示例·单选（填好后删）导线的主要作用是什么？",
    "单选",
    "基础，示例",
    "示例解析：导线的作用是传输电能，其余选项为干扰项。",
    ["A"],
    ["传输电能", "装饰线路", "储存电能", "测量温度"],
  ),
  questionRow(
    "示例·多选（填好后删）安全巡视应做到哪些？",
    "多选",
    "安全，示例",
    "示例解析：安全巡视应核对线路编号并佩戴防护用品。",
    ["AB"],
    ["佩戴防护用品", "核对线路编号", "跨越警戒区域", "跳过危险点记录"],
  ),
  questionRow(
    "示例·判断（填好后删）巡视前应确认天气和现场风险。",
    "判断",
    "安全，示例",
    "示例解析：巡视前应确认天气与现场风险，确保安全。",
    ["A"],
    ["正确", "错误"],
  ),
  questionRow(
    "示例·单空计算（填好后删）某量计算结果为【空1】。",
    "计算",
    "计算，示例",
    "示例解析：答案1 对应题干中的【空1】。",
    ["12.5"],
  ...
  questionRow(
    "示例·多空计算（填好后删）将 R=8Ω 的电阻和容抗 XC=6Ω 的电容器串联，接在频率为 f=50Hz、电压为 U=110V 的正弦交流电源上。电路中的电流为【空1】A，所消耗的有功功率为【空2】W。",
    "计算",
    "计算，多空，示例",
    "示例解析：两个填空分别对应答案1和答案2，每个空独立按误差比例判定。",
    ["11.0", "968.0"],
  ),
  questionRow(
    "示例·单空填空（填好后删）输电线路的主要作用是【空1】。",
    "填空",
    "填空，示例",
    "示例解析：同一空允许多个等价答案时，用 || 分隔。",
    ["传输电能||输送电能"],
  ),
  questionRow(
    "示例·多空填空（填好后删）三相交流电中，相邻两相电动势的相位差为【空1】°，我国电力系统工频为【空2】Hz。",
    "填空",
    "填空，多空，示例",
    "示例解析：答案1对应空1、答案2对应空2；每个空可分别设置多个等价答案。",
    ["120", "50||50赫兹"],
  ),
  questionRow(
    "示例·简答（填好后删）简述输电线路巡视前应完成的主要安全准备。",
    "简答",
    "简答，安全，示例",
    "示例解析：简答题由答题者对照参考答案自行判定正确或错误。",
    ["确认天气和现场风险，核对线路及作业信息，准备并检查必要的安全防护用品和巡视工具。"],
  ),
  questionRow(
    "示例·图片单选（填好后删）图中图标对应哪个应用？【图1】",
    "单选",
    "图片，示例",
    "示例解析：图片1 嵌入在本行最后一列，题干中的【图1】决定图片显示位置。",
    ["A"],
    ["拾卷", "浏览器", "相册", "日历"],
    `=DISPIMG("${templateImageId}",1)`,
  ),
];
...
const instructionRows = [
  ...
  ["题干", "必填。支持普通文字、公式、【图1】等图片占位符。计算题、填空题建议使用【空1】【空2】…标出答题位置。"],
  ["题型", "必填，只能填写：单选、多选、判断、计算、填空、简答。"],
  ["答案列", "所有题至少填写答案1。单选、多选、判断、简答只填写答案1；计算题、填空题按空位顺序填写答案1、答案2…，答案列之间不得留空。模板预留答案1~答案12。"],
  ["标签", "可选。多个标签使用中文逗号、英文逗号或顿号分隔。"],
  ["解析", "可选。该题的个人解析，导入时会写回为本机笔记（不会覆盖已有解析）。"],
  ["选项", "只有单选、多选、判断题需要填写选项。选项从全部答案列右侧的 A 列开始连续填写，不得断列；判断题必须依次为“正确、错误”。计算、填空、简答题不要填写选项。"],
  ...
  ["填空题", "每个空对应一个答案列；同一空存在多个可接受答案时，在同一个单元格中用 || 分隔，例如“电流||电流强度”。答案比较会忽略首尾空格并统一大小写/全半角。建议题干用【空1】【空2】…与答案列一一对应。"],
  ["简答题", "只填写答案1作为参考答案，其余答案列和全部选项列留空。练习时由答题者先作答，再对照参考答案自行判定正确或错误。"],
...
const sheets: XlsxSheet[] = [
  {
    name: "题库",
    rows: questionRows,
    rowHeights: questionRows.map((_, index) => index === questionRows.length - 1 ? 152 : 0),
    columnWidths: [58, 10, 22, 34, ...Array.from({ length: answerColumnCount }, () => 13), ...optionHeaders.map(() => 22), 28],
  },
  {
    name: "使用说明",
