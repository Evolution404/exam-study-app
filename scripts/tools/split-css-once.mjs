...
function splitAtTopLevel(source, target = 12000) {
  ...
  for (const line of lines) {
    chunk += line;
    scan(line);
    if (Buffer.byteLength(chunk) >= target && depth === 0 && !comment && !quote) {
...
const specs = [
  ...
  { old:"src/app/styles/sync-events.css", import:"./sync-events.css", dir:"src/app/styles", stem:"sync-events", rel:(n)=>`./sync-events-${n}.css` },
];
