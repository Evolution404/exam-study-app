#!/usr/bin/env node
/** One-shot migration from a legacy Sync v7 or v8 namespace to the current v9 wire protocol. */
...
function parseArgs(argv: readonly string[]): Args {
  ...
  if (!values.owner || !values.repo) {
    throw new Error("用法: npm run migrate:vault:v9 -- --owner <owner> --repo <repo> [--branch main] [--token <PAT>] [--api-base <url>] [--verify]");
  }
...
const result = await migrateVaultToSyncV9Protocol(
