...
const {
  ...
  syncWithGitHubV5,
} = await import("../lib/github-sync-v5");
import {
...
const settings = { owner: "v5-integration-owner", repo: "v5-integration-repo", branch: "main" };
const token = "v5-integration-token";
const bank: Bank = {
  id: "v5-bank",
  name: "送电线路工-初级工",
  questionCount: 1,
...
function event(id: string, sequence: number, deviceId = "v5-device", synced: 0 | 1 = 0): SyncEvent {
...
/**
 * A small Contents/blob API double. It intentionally exposes only the v5
 * endpoints, so a test cannot accidentally pass by using the old tree API.
 */
class V5RemoteDouble {
  ...
  head(): SyncHeadV5 | undefined {
    return this.headBytes ? JSON.parse(textDecoder.decode(this.headBytes)) as SyncHeadV5 : undefined;
  }

  setHead(head: SyncHeadV5) {
    const bytes = jsonBytes(head);
    const stored = this.putStored("sync/v5/head.json", bytes, 200);
    ...
    this.files.delete("sync/v5/head.json");
  ...
  async fetch(input: RequestInfo | URL, init: RequestInit = {}) {
    ...
    if (url.pathname.endsWith("/contents/sync/v5/head.json")) {
      if (method === "GET") {
      ...
      if (method === "PUT") {
        ...
        const stored = this.putStored("sync/v5/head.json", bytes, this.headSha ? 200 : 201);
        this.headBytes = stored.stored.bytes;
        this.headSha = stored.stored.sha;
        this.headEtag = `"head-${this.counter}"`;
        this.files.delete("sync/v5/head.json");
        ...
        return this.response({ content: { path: "sync/v5/head.json", sha: stored.stored.sha } }, stored.status, { ETag: this.headEtag });
      }
    }
...
async function descriptorForText(client: V5RemoteDouble, path: string, text: string): Promise<SyncHeadDescriptorV5> {
  const stored = client.putText(path, text).stored;
  return { path, blobSha: stored.sha, sha256: sha256(text), size: byteLength(text) };
}

async function eventPage(client: V5RemoteDouble, rows: SyncEvent[], deviceId = rows[0]?.deviceId ?? "remote-device"): Promise<SyncEventPageDescriptorV5> {
  const text = JSON.stringify({ formatVersion: 5, events: rows });
  const digest = sha256(text);
  const descriptor = await descriptorForText(client, `${SYNC_V5_EVENT_PREFIX}${digest}.json`, text);
...
async function archiveSegment<T extends { id: string }>(client: V5RemoteDouble, kind: "attempts" | "practice-runs", rows: T[], month = "2024-01"): Promise<SyncArchiveSegmentV5> {
  const text = JSON.stringify({ formatVersion: 5, kind, rows });
  const digest = sha256(text);
  const path = `${SYNC_V5_ARCHIVE_PREFIX}${kind}/${month}/${digest}.json`;
...
async function publishPackage(
  client: V5RemoteDouble,
  checkpoint: SyncCheckpointV5,
  options: { events?: SyncEvent[]; attemptSegments?: SyncArchiveSegmentV5[]; practiceRunSegments?: SyncArchiveSegmentV5[] } = {},
) {
  const checkpointText = JSON.stringify(checkpoint);
  const checkpointDescriptor = await descriptorForText(client, `${SYNC_V5_CHECKPOINT_PREFIX}${sha256(checkpointText)}.json`, checkpointText);
  const catalog = appendSyncArchiveSegmentsV5(createSyncArchiveCatalogV5(checkpoint.generatedAt), {
    attemptSegments: options.attemptSegments ?? [],
    practiceRunSegments: options.practiceRunSegments ?? [],
  });
  const catalogText = JSON.stringify(catalog);
  const catalogDescriptor = await descriptorForText(client, `${SYNC_V5_ARCHIVE_CATALOG_PREFIX}${sha256(catalogText)}.json`, catalogText);
...
async function seedArchiveLocal() {
  ...
  await db.banks.put(bank);
  await db.questions.put(question);
  const attempts = Array.from({ length: 2_501 }, (_, index) => attempt(`v5-attempt-${String(index).padStart(4, "0")}`, index));
  ...
  const runs = Array.from({ length: 601 }, (_, index) => practiceRun(`v5-run-${String(index).padStart(4, "0")}`, index));
...
const remote = new V5RemoteDouble();
remote.install();
...
assert.equal(remote.head()?.formatVersion, 5);
const initialHeadBytes = JSON.stringify(remote.head());
const repeated = await initializeGitHubVaultV5(settings, token);
...
remote.resetLogs();
const noChange = await syncWithGitHubV5(settings, token);
...
const uploaded = await syncWithGitHubV5(settings, token);
assert.equal(uploaded.pushed, 1);
assert.equal((await db.events.get(localEvent.id))?.synced, 1);
assert.equal(remote.immutablePutPaths.filter((path) => path.startsWith(SYNC_V5_EVENT_PREFIX)).length, 1);
...
const largeDefinition = {
  ...
  bankId: bank.id,
  bankIds: [bank.id, "bank-middle", "bank-senior", "bank-technician"],
  bankName: "4 个题库组合",
  mode: "randomAll" as const,
  modeLabel: "全部随机",
  questionIds: largeQuestionIds,
  questionTypes: Object.fromEntries(largeQuestionIds.map((id) => [id, "单选"])),
  shuffleOptions: false,
  optionOrders: {},
...
await resetLocalDatabase();
await restoreFromGitHubV5(settings, token);
...
async function runCasMergeCase(status: 409 | 422, eventId: string, sequence: number) {
  ...
  const result = await syncWithGitHubV5(settings, token);
...
remote.afterImmutablePut = async (path) => {
  if (!insertedLate && path.startsWith(SYNC_V5_EVENT_PREFIX)) {
    insertedLate = true;
    await db.events.put(lateEvent);
  }
};
const during = await syncWithGitHubV5(settings, token);
...
const secondDuring = await syncWithGitHubV5(settings, token);
...
await resetLocalDatabase();
remote.resetLogs();
const quick = await restoreFromGitHubV5(settings, token);
assert.equal(quick.formatVersion, 5);
...
assert.equal(remote.rawReads.some((path) => path.startsWith(`${SYNC_V5_ARCHIVE_PREFIX}attempts/`) || path.startsWith(`${SYNC_V5_ARCHIVE_PREFIX}practice-runs/`)), false, "quick restore must not download archive segments");
...
await assert.rejects(() => restoreFullHistoryFromGitHubV5(settings, token));
...
remote.resetLogs();
const full = await restoreFullHistoryFromGitHubV5(settings, token);
assert.equal(full.formatVersion, 5);
...
assert.equal(remote.rawReads.filter((path) => path.startsWith(`${SYNC_V5_ARCHIVE_PREFIX}attempts/`)).length, attemptSegments.length);
assert.equal(remote.rawReads.filter((path) => path.startsWith(`${SYNC_V5_ARCHIVE_PREFIX}practice-runs/`)).length, runSegments.length);

await db.delete();
console.log("sync v5 integration tests passed: fixed head, 304 no-op, upload/CAS merge, late events, quick restore and atomic segmented full restore");
