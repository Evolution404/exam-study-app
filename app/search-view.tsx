...
export function SearchView({
  ...
  onRelate: (questionId: string) => void;
  ...
  const [keywordMode, setKeywordMode] = useState<"plain" | "regex">("plain");
  ...
  if (!query.trim()) return <div className="search-page"><div className="page-heading compact"><div><p className="eyebrow">查题与整理</p><h1>搜索题库</h1><p>在顶部输入关键词，查找题干、选项、标签和个人解析。</p></div></div><section className="search-empty-page"><Search size={28} /><h2>输入关键词开始搜索</h2><p>搜索只读取本地题库，不会上传任何内容。</p>{history.length > 0 && <div className="search-history"><header><span><History size={15} />最近搜索</span><button onClick={clearHistory}>清除</button></header><div>{history.map((item) => <button key={item} onClick={() => onQueryChange(item)}>{item}</button>)}</div></div>}</section></div>;

  return <div className="search-page">
    <div className="search-page-heading"><div><p className="eyebrow">查题与整理</p><h1>“{query.trim()}”</h1><p>{result.error ? result.error : `找到 ${result.counts.单选 + result.counts.多选 + result.counts.判断} 道匹配题目`}</p></div><div className="search-heading-actions"><label>搜索范围<select value={scope} onChange={(event) => { setScope(event.target.value as "current" | "all"); setBankId("all"); }}><option value="current">首页已选题库</option><option value="all">全部题库</option></select></label><label>指定题库<select value={bankId} onChange={(event) => setBankId(event.target.value)}><option value="all">不指定</option>{banks.filter((bank) => scope === "all" || currentBankIds.includes(bank.id)).map((bank) => <option key={bank.id} value={bank.id}>{bank.name}</option>)}</select></label></div></div>
    <section className="search-toolbar"><div className="search-type-tabs">{(["全部", ...TYPE_ORDER] as TypeTab[]).map((type) => <button key={type} className={typeTab === type ? "active" : ""} onClick={() => { setTypeTab(type); setVisibleCount(50); }}>{type}<span>{type === "全部" ? result.counts.单选 + result.counts.多选 + result.counts.判断 : result.counts[type]}</span></button>)}</div><button className={`search-filter-toggle ${advancedOpen ? "active" : ""}`} onClick={() => setAdvancedOpen(!advancedOpen)}><Filter size={16} />筛选条件</button></section>
    ...
    {data === undefined ? <div className="search-loading"><LoaderCircle className="spin" />正在读取本地题库…</div> : result.error ? <div className="search-no-result"><CircleAlert /><h2>{result.error}</h2></div> : result.entries.length ? <>
      ...
      <div className="search-result-list">{visibleEntries.map(({ question, metric, hasNote }, index) => <article key={question.id} className={selectedIds.includes(question.id) ? "selected" : ""}><label className="result-checkbox"><input type="checkbox" checked={selectedIds.includes(question.id)} onChange={() => setSelectedIds(selectedIds.includes(question.id) ? selectedIds.filter((id) => id !== question.id) : [...selectedIds, question.id])} /><span>{index + 1}</span></label><button className="search-result-main" onClick={() => setDetailQuestionId(question.id)}><div><span className="result-type">{question.type}</span><span>{question.bankName}</span>{question.tags.map((item) => <em key={item}>{item}</em>)}</div><h2>{question.stem}</h2><p>难度 {metric.difficulty} · 作答 {metric.total} 次 · 错误 {metric.wrong} 次{hasNote ? " · 已有个人解析" : ""}</p></button><ChevronRight size={18} /></article>)}</div>
    ...
    {detailQuestionId && <SearchQuestionDetail questionId={detailQuestionId} onClose={() => { setDetailQuestionId(undefined); onFocusHandled(); }} onStart={(question) => setPracticeSource({ questions: [question], label: "单题练习" })} onRelate={onRelate} onNotice={onNotice} />}
...
function SearchQuestionDetail({ questionId, onClose, onStart, onRelate, onNotice }: { questionId: string; onClose: () => void; onStart: (question: Question) => void; onRelate: (questionId: string) => void; onNotice: (message: string) => void }) {
