...
import { loadImageAssetV6, SharedQuestionEditor, toQuestionViewModel, type QuestionViewModel } from "@/app/question-editor";
...
import type { BankV6, QuestionTypeV6, AttemptStatsV6 } from "@/lib/v6-types";
type Bank = BankV6;
...
export function SearchView({
  ...
  const data = useLiveQuery(async () => {
    ...
    const [rawStats, notes, roundProgress] = await Promise.all([dbV6.attemptStats.toArray(), dbV6.notes.toArray(), dbV6.reviewRoundProgress.toArray()]);
    ...
    return { questions, attemptStats, notes: notes.filter((note) => ids.has(note.questionId)), roundProgress: roundProgress.filter((row) => ids.has(row.questionId)) };
  }, [bankKey]);
  ...
  const result = useMemo(() => {
    ...
    if (!normalized) return { entries: [] as Array<{ question: Question; metric: ReturnType<typeof summarizeAttemptStats>; hasNote: boolean }>, counts: { 单选: 0, 多选: 0, 判断: 0, 计算: 0 }, error: "" };
  ...
  }, [data, query, keywordMode, tag, status, noteFilter, difficultyMin, difficultyMax, attemptsMin, attemptsMax, wrongMin, wrongMax, lastFrom, lastTo, typeTab, wrongRemovalStreak, normalizedScope, referenceTime]);
