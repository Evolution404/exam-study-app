...
export function ShortcutSetting({ value, onChange }: { value: KeyboardShortcuts; onChange: (value: KeyboardShortcuts) => void }) {
  ...
  return <section className="preference-card shortcut-preference-card">
    ...
    <div className={`shortcut-editor ${value.enabled ? "" : "disabled"}`} aria-disabled={!value.enabled}>
      ...
      <div className="shortcut-navigation-grid"><div><strong>上一题</strong><span>{keyInput("previousKeys", 0, "上一题快捷键一")}{keyInput("previousKeys", 1, "上一题快捷键二")}</span></div><div><strong>下一题</strong><span>{keyInput("nextKeys", 0, "下一题快捷键一")}{keyInput("nextKeys", 1, "下一题快捷键二")}</span></div><div className="shortcut-confirm-row"><strong>确认答案</strong><span className="shortcut-fixed-key"><kbd>Enter</kbd></span></div></div>
