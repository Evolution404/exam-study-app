...
import {
  BookOpen, Brain, Check, ChevronRight, CircleAlert, Cloud,
  FileUp, GitBranch, Home, Library, Link2, LoaderCircle, Menu,
  NotebookPen, RotateCcw, Search, Sparkles, Target, X,
} from "lucide-react";
...
type View = "home" | "banks" | "wrong" | "relations" | "settings" | "practice";
...
export function StudyApp() {
  ...
  async function startPractice(filter: "all" | "wrong" = "all") {
    let questions: Question[];
    if (filter === "wrong") {
      const attempts = await db.attempts.where("correct").equals(0).toArray();
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <div className="content">
          {view === "home" && <Dashboard stats={stats} banks={banks} onImport={() => fileRef.current?.click()} onStart={() => startPractice()} />}
          {view === "banks" && <BanksView banks={banks} query={query} onImport={() => fileRef.current?.click()} onStart={() => startPractice()} />}
          {view === "wrong" && <WrongView onStart={() => startPractice("wrong")} />}
          ...
          {view === "practice" && queue[queueIndex] && (
            <Practice question={queue[queueIndex]} index={queueIndex} total={queue.length} onExit={() => setView("home")} onNext={() => {
...
function Dashboard({ stats, banks, onImport, onStart }: {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">今天也向前一点</p><h1>把知识，练成下意识。</h1><p>题库留在你的设备里，记录完整，随时继续。</p></div><button className="primary" onClick={onStart}><Brain size={18} />开始练习</button></div>
    <section className="hero-grid">
      <article className="focus-card">
        <div className="focus-copy"><span className="section-kicker">今日计划</span><h2>{stats.questions ? "来一组 30 题" : "先导入第一份题库"}</h2><p>{stats.questions ? "混合题型随机抽取，答完立即记录。" : "支持当前项目的 q / ans / a JSON 格式。"}</p><button onClick={stats.questions ? onStart : onImport}>{stats.questions ? "开始这一组" : "选择 JSON 文件"}<ChevronRight size={17} /></button></div>
...
function BanksView({ banks, query, onImport, onStart }: { banks: Array<{ id: string; name: string; questionCount: number; importedAt: string }>; query: string; onImport: () => void; onStart: () => void }) {
  ...
  return <><div className="page-heading compact"><div><p className="eyebrow">我的资料</p><h1>题库</h1><p>导入、版本化并随时开始一组练习。</p></div><button className="primary" onClick={onImport}><FileUp size={18} />导入题库</button></div>
    {filtered.length ? <div className="library-grid">{filtered.map((bank, index) => <article className="library-card" key={bank.id}><span className={`bank-icon large tone-${index % 3}`}><Library size={22} /></span><span className="bank-ready">本地可用</span><h2>{bank.name}</h2><p>{bank.questionCount.toLocaleString()} 道题 · {formatDate(bank.importedAt)} 导入</p><button onClick={onStart}>随机练习<ChevronRight size={17} /></button></article>)}</div> : <EmptyImport onImport={onImport} />}
  </>;
}

function WrongView({ onStart }: { onStart: () => void }) {
  const count = useLiveQuery(async () => {
    const rows = await db.attempts.where("correct").equals(0).toArray();
    return new Set(rows.map((row) => row.questionId)).size;
  }, []) ?? 0;
  return <div className="center-panel"><span className="center-icon warning"><CircleAlert /></span><p className="eyebrow">错题回炉</p><h1>{count ? `${count} 道题等你攻克` : "这里还没有错题"}</h1><p>{count ? "重新作答，连续答对后再移出重点复习。" : "每次作答都会自动记录，答错的题会出现在这里。"}</p><button className="primary" onClick={onStart} disabled={!count}><RotateCcw size={18} />开始错题练习</button></div>;
}
...
function Practice({ question, index, total, onNext, onExit }: { question: Question; index: number; total: number; onNext: () => void; onExit: () => void }) {
  ...
  const correct = submitted && selected.sort().join("") === question.answer.split("").sort().join("");

  function toggle(letter: string) {
    if (submitted) return;
    if (question.type === "多选") setSelected(selected.includes(letter) ? selected.filter((item) => item !== letter) : [...selected, letter]);
    else setSelected([letter]);
  }
  async function submit() {
    if (!selected.length) return;
    const value = selected.sort().join("");
    await recordAttempt({ questionId: question.id, bankId: question.bankId, selected: value, correct: value === question.answer, elapsedMs: Date.now() - startedAt });
    setSubmitted(true);
  }
  function next() { setSelected([]); setSubmitted(false); setDraft(null); setStartedAt(Date.now()); onNext(); }
  return <div className="practice-layout"><section className="question-card"><div className="practice-head"><button className="icon-button" onClick={onExit}><X size={19} /></button><div><span>{index + 1} / {total}</span><i><b style={{ width: `${(index + 1) / total * 100}%` }} /></i></div><span className="type-chip">{question.type}</span></div>
    <div className="question-body"><div className="question-meta"><span>{question.bankName}</span>{question.tags.map((tag) => <em key={tag}>{tag}</em>)}</div><h1>{question.stem}</h1><div className="options">{question.options.map((option, optionIndex) => { const letter = String.fromCharCode(65 + optionIndex); const isAnswer = submitted && question.answer.includes(letter); const isWrong = submitted && selected.includes(letter) && !question.answer.includes(letter); return <button key={letter} className={`${selected.includes(letter) ? "selected" : ""} ${isAnswer ? "right" : ""} ${isWrong ? "wrong" : ""}`} onClick={() => toggle(letter)}><span>{letter}</span><p>{option}</p>{isAnswer && <Check size={18} />}{isWrong && <X size={18} />}</button>; })}</div>
      {submitted && <div className={`result-box ${correct ? "success" : "error"}`}><strong>{correct ? "回答正确" : "这次没有答对"}</strong><p>正确答案：{question.answer}｜{answerText(question)}</p></div>}
    </div><div className="practice-actions">{submitted ? <button className="primary" onClick={next}>下一题<ChevronRight size={18} /></button> : <button className="primary" disabled={!selected.length} onClick={submit}>确认答案</button>}</div></section>
