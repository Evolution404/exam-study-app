...
export interface V6PracticeFilter {
  bankIds: string[];
  mode: V6PracticeMode;
  types: QuestionTypeV6[];
  tags: string[];
  tagMatch: "any" | "all";
...
const baseModes: Array<{ id: V6PracticeMode; title: string; detail: string; icon: typeof Shuffle }> = [
...
const questionTypes: QuestionTypeV6[] = ["单选", "多选", "判断", "计算"];
...
export function PracticeSetupView({ banks, currentBankIds, onBankChange, onStart, hideHeading = false, groupSize = 30, defaultOrder = "sequential", progressScope = { type: "rolling", days: 90 }, rounds = [] }: {
  ...
  onStart: (filter: V6PracticeFilter) => void;
  hideHeading?: boolean;
  groupSize?: number;
  defaultOrder?: V6PracticeFilter["order"];
  progressScope?: ProgressScope;
  rounds?: readonly ReviewRound[];
}) {
  const [bankIds, setBankIds] = useState(currentBankIds);
  const [mode, setMode] = useState<V6PracticeMode>("sequential");
  const [types, setTypes] = useState<QuestionTypeV6[]>(questionTypes);
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  const [tagMatch, setTagMatch] = useState<"any" | "all">("any");
  const [status, setStatus] = useState<V6PracticeFilter["status"]>("all");
  const [order, setOrder] = useState<V6PracticeFilter["order"]>(defaultOrder);
  const [limit, setLimit] = useState<number | null>(null);
  const [customRandomCount, setCustomRandomCount] = useState(String(groupSize));
  const [keyword, setKeyword] = useState("");
  const [keywordMode, setKeywordMode] = useState<V6PracticeFilter["keywordMode"]>("plain");
  ...
  const dataset = useLiveQuery(async () => {
    const [questions, stats, roundsProgress] = await Promise.all([
      getQuestionsForBanksV6(bankIds),
  ...
  function toggleType(type: QuestionTypeV6) { setTypes(types.includes(type) ? types.filter((item) => item !== type) : [...types, type]); }
