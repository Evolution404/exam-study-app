...
export function PracticeSetupView({ banks, currentBankIds, onBankChange, onStart }: {
  ...
  const [lastAttemptRange, setLastAttemptRange] = useState<PracticeFilter["lastAttemptRange"]>("any");
  ...
  function start() {
    const filter: PracticeFilter = {
      ...
      lastAttemptRange: mode === "advanced" ? lastAttemptRange : "any",
    };
    onStart(filter);
  }
  ...
  const metricError = totalMin !== null && totalMax !== null && totalMin > totalMax
    ...
    : wrongMin !== null && wrongMax !== null && wrongMin > wrongMax
      ? "错误次数的最少值不能大于最多值"
      : "";
  const disabled = !bankIds.length || (mode === "tag" && !selectedTags.length) || (mode === "advanced" && (!types.length || Boolean(regexError) || Boolean(metricError)));
  return <>
    ...
    <section className="practice-setup-card">
      ...
      {mode === "advanced" && <>
        ...
        <div className="advanced-grid">
          ...
          <label>距上次作答<select value={lastAttemptRange} onChange={(event) => setLastAttemptRange(event.target.value as PracticeFilter["lastAttemptRange"])}><option value="any">不限时间</option><option value="never">从未作答</option><option value="within1">最近 1 天内</option><option value="within7">最近 7 天内</option><option value="within30">最近 30 天内</option><option value="within90">最近 90 天内</option><option value="over7">超过 7 天</option><option value="over30">超过 30 天</option><option value="over90">超过 90 天</option></select></label>
