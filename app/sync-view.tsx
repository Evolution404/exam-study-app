...
export function SyncView({ pending, onNotice, onRestored }: { pending: number; onNotice: (message: string) => void; onRestored: (message: string) => void }) {
  ...
  return <>
    ...
    <section className="settings-card sync-events-page"><SyncEventManager items={changeSetItems} syncing={syncing} progress={operationProgress} showBatchSections onRefresh={() => undefined} onSyncNow={sync} onEdit={editChangeSet} onDelete={async (id, options) => { await discardManagedChangeSetV7(id, options); }} /></section>
