...
export function SyncView({ pending, onNotice }: { pending: number; onNotice: (message: string) => void }) {
  ...
  async function restoreFromCache() {
    ...
    if (!window.confirm(`将丢弃当前浏览器中的全部本地修改，并立即恢复到 ${new Date(lastCache.cachedAt).toLocaleString("zh-CN")} 缓存的远程状态。此操作不会访问 GitHub，确定继续吗？`)) return;
    try {
  ...
  return <>
    ...
    <section className="restore-card"><div className="restore-icon"><DatabaseBackup /></div><div><span className="section-kicker">快速回到上次远程状态</span><h2>放弃本地修改</h2><p>{lastCache ? `本机已缓存 ${new Date(lastCache.cachedAt).toLocaleString("zh-CN")} 的远程状态，可立即恢复且不访问网络。` : "成功同步一次后，本机会保留最近的远程状态，用于离线快速恢复。"}</p></div><div className="restore-actions"><button className="danger-button" disabled={!lastCache || syncing || restoring || restoringCache} onClick={restoreFromCache}>{restoringCache ? <LoaderCircle className="spin" size={18} /> : <DatabaseBackup size={18} />}{restoringCache ? "正在恢复…" : "恢复上次远程缓存"}</button><button className="secondary-action" disabled={!ready || syncing || restoring || restoringCache} onClick={restoreFromRemote}>{restoring ? <LoaderCircle className="spin" size={18} /> : <CloudDownload size={18} />}{restoring ? "正在下载并重建…" : "重新从 GitHub 下载"}</button></div></section>
  </>;
}
