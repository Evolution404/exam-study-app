...
- 公开同步协议：Sync v7，唯一可变入口 `sync/v7/head.json`；UI 只通过 `lib/github-sync.ts` 门面访问同步。
...
lib/           # 领域逻辑，不依赖 React 页面；目前平铺，后续可继续分层
...
- `lib/github-sync.ts` 是 UI 唯一公开同步门面，只委托 v7 transport。
- GitHub API 代理源码在 `proxy/`；`functions/api-github/[[path]].js` 由构建自动生成，不手写。

## 4. 关键文件

- 数据模型：`lib/v6-types.ts`, `lib/db-v6.ts`, `lib/app-data-v6.ts`
- 同步协议：`lib/sync-v7-head.ts`, `lib/sync-v7-codec.ts`, `lib/sync-v7-payload.ts`,
  `lib/change-set-v7.ts`, `lib/change-set-v7-projection.ts`, `lib/change-set-v7-queue.ts`,
  `lib/github-v7-remote.ts`, `lib/github-sync-v7.ts`, `lib/github-sync.ts`
- 进度与轮次：`lib/progress-scope.ts`, `app/practice/progress-scope-setting.tsx`,
  `app/practice/review-round-manager.tsx`, `app/practice/practice-setup.tsx`
- 富内容与图片：`lib/image-assets.ts`, `lib/image-asset-cache.ts`, `lib/image-dimensions.ts`,
  `app/bank/content-block-editor.tsx`, `app/bank/content-block-renderer.tsx`, `app/ui/asset-image.tsx`
- 导入导出：`lib/xlsx-import.ts`, `lib/xlsx-export.ts`, `lib/question-bank-file-import.ts`,
  `lib/question-bank-export.ts`, `lib/question-bank-bundle.ts`
