...
import { studyDb, getDeviceId, makeId, nowIso, uniqueStrings } from "./db-core";
...
function rawQuestionRows(raw: unknown): { name?: string; rows: ImportedQuestionRow[] } {
  if (Array.isArray(raw)) return { rows: raw as ImportedQuestionRow[] };
  ...
  return { name: typeof record.name === "string" ? record.name : undefined, rows: record.questions as ImportedQuestionRow[] };
}
...
function solutionMatchesType(type: QuestionType, solution: QuestionSolution): boolean {
...
function importDraft(row: ImportedQuestionRow): StructuredQuestionDraft | undefined {
  ...
  const type: QuestionType = rawType;
...
export async function importQuestionBank(fileName: string, raw: unknown, options?: { targetBankId?: string; imageAssets?: readonly ImageAsset[] }): Promise<Bank & { importedCount: number }> {
  const parsed = rawQuestionRows(raw);
  const rows = parsed.rows.map(importDraft).filter((row): row is StructuredQuestionDraft => Boolean(row));
  ...
  return studyDb.transaction("rw", [studyDb.banks, studyDb.questions, studyDb.bankQuestionMemberships, studyDb.bankQuestionStats, studyDb.tombstones, studyDb.changeSets, studyDb.notes, studyDb.syncMeta], async () => {
    ...
    if (options?.targetBankId) {
      ...
      const existingBank = await studyDb.banks.get(bankId);
      bank = existingBank ? {
        ...existingBank,
    ...
    const materialised: Array<{ question: Question; membership: BankQuestionMembership; isNewMembership: boolean }> = [];
    const materialisedNotes: Note[] = [];
    let sortOrder = await studyDb.bankQuestionMemberships.where("bankId").equals(bank.id).count();
    for (const { draft, provisional } of prepared) {
      ...
      const existingMembership = await studyDb.bankQuestionMemberships.get(membershipPrimaryKey(bank.id, question.id));
      ...
      if (draft.note?.trim() && !(await studyDb.notes.get(question.id))) {
    ...
    for (const item of materialised) {
      if (!(await studyDb.questions.get(item.question.id))) await studyDb.questions.put(item.question);
    ...
    if (materialisedNotes.length) {
      await enqueueChangeSet(materialisedNotes.map((note) => ({ kind: "note.upserted" as const, note })), timestamp);
    }
