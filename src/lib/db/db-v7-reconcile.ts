...
function emitTiming(
  ...
  options.onTiming?.({ ...timing, mode: "full" });
}
...
export async function reconcileV7Projection(
  ...
  const bankPlan = await planTableTimed(dbV7.banks, state.banks, (row) => row.id, options);
  const folderPlan = await planTableTimed(dbV7.bankFolders, state.bankFolders, (row) => row.id, options);
  const questionPlan = await planTableTimed(dbV7.questions, state.questions, (row) => row.id, options);
  const membershipPlan = await planTableTimed(dbV7.bankQuestionMemberships, memberships, (row) => row.key, options);
  const attemptPlan = await planTableTimed(dbV7.attempts, state.attempts, (row) => row.id, options);
  const attemptStatsPlan = await planTableTimed(dbV7.attemptStats, state.attemptStats, (row) => row.questionId, options);
  const dailyStatsPlan = await planTableTimed(dbV7.attemptDailyStats, state.attemptDailyStats, (row) => row.key, options);
  const notePlan = await planTableTimed(dbV7.notes, state.notes, (row) => row.questionId, options);
  const practiceRunPlan = await planTableTimed(dbV7.practiceRuns, state.practiceRuns, (row) => row.id, options);
  const practiceStatsPlan = await planTableTimed(dbV7.practiceRunStats, state.practiceRunStats, (row) => row.key, options);
  const groupPlan = await planTableTimed(dbV7.questionGroups, state.questionGroups, (row) => row.id, options);
  const roundPlan = await planTableTimed(dbV7.reviewRounds, state.reviewRounds, (row) => row.id, options);
  const roundProgressPlan = await planTableTimed(dbV7.reviewRoundProgress, state.reviewRoundProgress, (row) => row.key, options);
  const tombstonePlan = await planTableTimed(dbV7.tombstones, state.tombstones, (row) => row.key, options);
  const imagePlan = await planImageAssetsTimed(state.imageAssets, options);
  ...
  return dbV7.transaction("rw", transactionTables, async () => {
    ...
    try {
      ...
      await applyPlan(dbV7.banks, bankPlan, { put: "更新题库", remove: "清理题库" }, progress, options);
      await applyPlan(dbV7.bankFolders, folderPlan, { put: "更新文件夹", remove: "清理文件夹" }, progress, options);
      await applyPlan(dbV7.questions, questionPlan, { put: "更新题目", remove: "清理题目" }, progress, options);
      await applyPlan(dbV7.bankQuestionMemberships, membershipPlan, { put: "更新题库关系", remove: "清理题库关系" }, progress, options);
      await applyPlan(dbV7.attempts, attemptPlan, { put: "更新作答记录", remove: "清理作答记录" }, progress, options);
      await applyPlan(dbV7.attemptStats, attemptStatsPlan, { put: "更新学习统计", remove: "清理学习统计" }, progress, options);
      await applyPlan(dbV7.attemptDailyStats, dailyStatsPlan, { put: "更新每日统计", remove: "清理每日统计" }, progress, options);
      await applyPlan(dbV7.notes, notePlan, { put: "更新解析笔记", remove: "清理解析笔记" }, progress, options);
      await applyPlan(dbV7.practiceRuns, practiceRunPlan, { put: "更新练习记录", remove: "清理练习记录" }, progress, options);
      await applyPlan(dbV7.practiceRunStats, practiceStatsPlan, { put: "更新练习统计", remove: "清理练习统计" }, progress, options);
      await applyPlan(dbV7.questionGroups, groupPlan, { put: "更新题组", remove: "清理题组" }, progress, options);
      await applyPlan(dbV7.reviewRounds, roundPlan, { put: "更新复习轮次", remove: "清理复习轮次" }, progress, options);
      await applyPlan(dbV7.reviewRoundProgress, roundProgressPlan, { put: "更新轮次进度", remove: "清理轮次进度" }, progress, options);
      await applyPlan(dbV7.tombstones, tombstonePlan, { put: "更新删除标记", remove: "清理删除标记" }, progress, options);
