/* Browser-safe SHA-256 fallback for insecure LAN origins where WebCrypto's
 * subtle API is unavailable. The result is byte-for-byte compatible with
...
export async function sha256DigestHex(bytes: Uint8Array): Promise<string> {
  ...
  if (subtle) {
    try {
      ...
      // Some embedded browsers expose subtle but reject digest in an insecure
      // context. The deterministic fallback below preserves functionality.
    }
  }
  return sha256HexBytes(input);
}
