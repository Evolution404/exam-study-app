import { studyDb, listChangeSets } from "../db/db";
...
import { saveHeadCache } from "./sync-cache";
import type { SyncHead, SyncDeviceWatermark } from "./sync-head-types";
...
export const SYNC_DEVICE_RETIRE_DAYS = 90;
 ...
 *  confirmed. */
export function reclaimableTombstones(
  tombstones: readonly Tombstone[],
  input: { devices: Record<string, SyncDeviceWatermark>; headCursors: Record<string, number>; selfDeviceId: string; now?: string },
): { keep: Tombstone[]; dropped: number } {
  const now = input.now ?? new Date().toISOString();
  const retireCutoff = Date.parse(now) - SYNC_DEVICE_RETIRE_DAYS * 86_400_000;
...
export async function publishDeviceWatermark(client: GitHubRemote, settings: GitHubSettings, deviceId: string, cursors: Record<string, number>): Promise<void> {
  ...
  if (!advanced) return;
  const nextHead: SyncHead = { ...read.head, devices: { ...(read.head.devices ?? {}), [deviceId]: { cursors, syncedAt: new Date().toISOString() } } };
 ...
 *  full re-install. */
export function installFingerprint(cache: SyncHeadCache): string {
...
export function projectionNeedsInstall(installedFingerprint: string | undefined, cache: SyncHeadCache, unseenCount: number, blockedCount: number): boolean {
...
const SYNC_COMMITTED_KEEP_RECENT = 500;
 ...
 */
export async function pruneCommittedChangeSets(cursors: Record<string, number>, keepRecent = SYNC_COMMITTED_KEEP_RECENT): Promise<void> {
  ...
  const excess = absorbed
    ...
    .slice(keepRecent);
  if (excess.length) await studyDb.changeSets.bulkDelete(excess.map((record) => record.id));
}
