/**
 * Immutable-ref offloading for sync events.
 *
 * A change-set is the atomic unit of the journal and its encoded form must
 * fit inside a bounded segment (SYNC_MAX_EVENT_BYTES, 256 KiB). A
 ...
 * the current immutable object namespace, and the segment event becomes a thin stub
 ...
 * (pull), so the domain reducer (reduceChangeSet) never observes a stub.
 */
import { sha256DigestHex } from "../crypto/sha256";
import { SYNC_OBJECT_PREFIX, type SyncImmutableRef, type SyncPublicationFile } from "./sync-head-types";
import { createSyncObjectRef } from "./sync-head-operations";
 ...
 * only ceiling is SYNC_MAX_OBJECT_BYTES (32 MiB) — orders of magnitude above
 ...
 */
export const SYNC_INLINE_EVENT_BUDGET = 128 * 1024;
...
export interface SyncOffloadResult {
...
export function offloadedRefOf(event: unknown): SyncImmutableRef | undefined {
...
export function isOffloadedSyncEvent(event: unknown): boolean {
 ...
 */
export async function offloadSyncEvents(
  events: readonly Record<string, unknown>[],
  budget: number = SYNC_INLINE_EVENT_BUDGET,
): Promise<SyncOffloadResult> {
  const wire: Array<Record<string, unknown>> = [];
  const objects: SyncPublicationFile[] = [];
  for (const event of events) {
    ...
    const path = `${SYNC_OBJECT_PREFIX}${sha256}.json`;
    const ref = createSyncObjectRef(path, sha256, bytes.byteLength, "object");
 ...
 * content hash and size. The caller supplies the remote fetcher
 * reading by content-addressed path), keeping this module transport-agnostic
 * and unit-testable without a network.
 */
export async function hydrateSyncEvents<T>(
  events: readonly T[],
  fetchObject: (ref: SyncImmutableRef) => Promise<Uint8Array | ArrayBuffer | string>,
): Promise<T[]> {
  const result: T[] = [];
  for (const event of events) {
    ...
    if (digest !== ref.sha256 || raw.byteLength !== ref.size) {
      throw new Error(`immutable sync object ${ref.path} failed its sha256/size integrity check`);
    }
