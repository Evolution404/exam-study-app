...
export function mergeSyncSegments(existing: readonly SyncSegmentDescriptor[], additions: readonly SyncSegmentDescriptor[], vaultId?: string): SyncSegmentDescriptor[] {
  ...
  for (const segment of [...existing, ...additions]) {
    ...
    if (prior && !sameSyncSegment(prior, segment)) throw new Error(`v9 segment replay-key collision: ${key}`);
    const pathPrior = byPath.get(segment.path);
    if (pathPrior && !sameSyncSegment(pathPrior, segment)) throw new Error(`v9 segment path collision: ${segment.path}`);
  ...
  if (result.length > SYNC_MAX_SEGMENT_COUNT) throw new Error("v9 segments exceed the bounded index limit");
  const hotBytes = result.reduce((sum, segment) => sum + segment.size, 0);
  if (hotBytes > SYNC_MAX_HOT_BYTES) throw new Error("v9 segments exceed the aggregate hot-window byte limit; compact explicitly first");
  return result;
}
...
export function encodeSyncEvent(event: unknown): Uint8Array {
  ...
  if (bytes.byteLength > SYNC_MAX_EVENT_BYTES) throw new RangeError(`v9 event exceeds ${SYNC_MAX_EVENT_BYTES} UTF-8 bytes; store its payload as an immutable ref`);
  return bytes;
}

export function encodeSyncSegment<T>(segment: SyncSegment<T>): Uint8Array {
  if (!segment || segment.formatVersion !== SYNC_FORMAT_VERSION) throw new TypeError("v9 segment formatVersion must be 9");
  ...
  if (!Array.isArray(segment.events) || segment.events.length < 1 || segment.events.length > SYNC_MAX_SEGMENT_EVENT_COUNT) throw new RangeError(`v9 segment must contain 1-${SYNC_MAX_SEGMENT_EVENT_COUNT} events`);
  ...
  if (bytes.byteLength > SYNC_MAX_SEGMENT_BYTES) throw new RangeError(`v9 segment exceeds ${SYNC_MAX_SEGMENT_BYTES} bytes`);
  return bytes;
}
...
export function decodeSyncSegment<T = unknown>(bytes: SyncBytes, expected?: { vaultId?: string; generation?: number; ordinal?: number }): SyncSegment<T> {
  ...
  if (!isRecord(value) || value.formatVersion !== SYNC_FORMAT_VERSION || !Array.isArray(value.events)) throw new Error("invalid v9 segment envelope");
...
export function paginateSyncEvents<T>(events: readonly T[]): Array<{ events: T[]; bytes: Uint8Array; size: number; count: number }> {
  ...
  for (const event of events) {
    ...
    if (!tooLarge && current.length > 0) {
      try { tooLarge = encodeSyncSegment({ formatVersion: 9, vaultId: "v9-pagination", generation: 0, ordinal: 0, metadata: { vaultId: "v9-pagination", createdAt: "2026-01-01T00:00:00.000Z" }, cursors: {}, events: candidate }).byteLength > SYNC_MAX_SEGMENT_BYTES; } catch { tooLarge = true; }
    }
    if (tooLarge) {
      const bytes = encodeSyncSegment({ formatVersion: 9, vaultId: "v9-pagination", generation: 0, ordinal: 0, metadata: { vaultId: "v9-pagination", createdAt: "2026-01-01T00:00:00.000Z" }, cursors: {}, events: current });
      result.push({ events: current, bytes, size: bytes.byteLength, count: current.length });
      current = [event];
      encodeSyncSegment({ formatVersion: 9, vaultId: "v9-pagination", generation: 0, ordinal: 0, metadata: { vaultId: "v9-pagination", createdAt: "2026-01-01T00:00:00.000Z" }, cursors: {}, events: current });
...
export function orderSyncSegments<T>(segments: readonly SyncReplaySegment<T>[]): SyncReplaySegment<T>[] {
  ...
  for (let index = 1; index < copy.length; index += 1) {
    if (compareSyncSegmentOrder(copy[index - 1], copy[index]) === 0) throw new Error("v9 replay contains duplicate generation/ordinal");
  }
  return copy;
}
...
function assertCompactionPlan(value: SyncCompactionPlan): void {
  ...
  if (value.reason === "hot-window-overflow" && value.hotBytes <= SYNC_MAX_HOT_BYTES) throw new Error("v9 hot-window-overflow compaction requires aggregate bytes above the threshold");
}
