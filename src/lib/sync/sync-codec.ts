/**
 * Transport/storage envelope codec for sync objects (segments, checkpoints,
...
export function syncCompressionEnabled(): boolean {
...
export async function encodeSyncJsonBytes(bytes: Uint8Array): Promise<Uint8Array> {
...
export async function decodeSyncJsonBytes(bytes: Uint8Array): Promise<Uint8Array> {
  if (!isZlibEnvelope(bytes)) return bytes;
  return inflate(bytes);
}

/** Text-level convenience wrappers over the byte codec. */
export async function encodeSyncJson(text: string): Promise<Uint8Array> {
  return encodeSyncJsonBytes(TEXT_ENCODER.encode(text));
}

export async function decodeSyncJson(bytes: Uint8Array): Promise<string> {
  return TEXT_DECODER.decode(await decodeSyncJsonBytes(bytes));
}
