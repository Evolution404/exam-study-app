...
import {
  ...
  type ClaimedBatch,
  type ChangeSetDependency,
  type ChangeSetEntityRef,
  type ChangeSetMutation,
  type ChangeSetPolicy,
  type ChangeSetPublicationState,
  type ChangeSetQueueBlocker,
  type ChangeSetQueuePlan,
  type ChangeSetReplayPhase,
  type ChangeSet,
} from "./change-set-types";
import { mutationEntityRefs } from "./change-set-refs";

const encoder = new TextEncoder();
const phases: readonly ChangeSetReplayPhase[] = ["assets", "folders", "banks", "questions", "memberships", "runs", "answers", "annotations", "deletes"];
...
export function summarizeChangeSet(changeSet: ChangeSet): string {
  ...
  if (changeSet.mutations.length > 1) return `批量操作 ${changeSet.mutations.length} 项：${summarizeMutation(mutation)}`;
  return summarizeMutation(mutation);
}

function summarizeMutation(mutation: ChangeSetMutation): string {
...
export function replayPhaseForMutation(mutation: ChangeSetMutation): ChangeSetReplayPhase {
...
function refsFor(mutation: ChangeSetMutation, type: string): string[] {
...
export function dependenciesForChangeSet(changeSet: ChangeSet, queued: readonly ChangeSet[] = []): ChangeSetDependency {
  ...
  const mutationPhases = mutations.map(replayPhaseForMutation);
...
export const getChangeSetDependencies = dependenciesForChangeSet;

export function policyForChangeSet(changeSet: ChangeSet, publication: ChangeSetPublicationState = { state: "pending" }): ChangeSetPolicy {
  void changeSet;
...
export const canEditChangeSet = (changeSet: ChangeSet, publication?: ChangeSetPublicationState): boolean => policyForChangeSet(changeSet, publication).editable;
export const canCancelChangeSet = (changeSet: ChangeSet, publication?: ChangeSetPublicationState): boolean => policyForChangeSet(changeSet, publication).cancellable;

function compareChangeSets(a: ChangeSet, b: ChangeSet): number {
...
export async function digestChangeSetBatch(changeSets: readonly ChangeSet[]): Promise<string> {
  const ordered = [...changeSets].sort(compareChangeSets).map((item) => item.digest);
  return sha256(encoder.encode(JSON.stringify(ordered)));
}

function mutationCreatedRefs(mutation: ChangeSetMutation): ChangeSetEntityRef[] {
  switch (mutation.kind) {
...
export function dependentChangeSetIds(target: ChangeSet, queued: readonly ChangeSet[]): string[] {
  ...
  // the O(n) conflict scan that dependenciesForChangeSet would otherwise run
  // per candidate per pass (this made the UI quadratic-to-cubic as events grew).
  const requiresByCandidate = new Map(queued.map((candidate) => [candidate.id, new Set(dependenciesForChangeSet(candidate).requires)]));
  let changed = true;
  while (changed) {
    changed = false;
...
export async function planChangeSetQueue(changeSets: readonly ChangeSet[], existingEntityRefs: readonly ChangeSetEntityRef[] = []): Promise<ChangeSetQueuePlan> {
  ...
  const blockers: ChangeSetQueueBlocker[] = [];
  for (const item of ordered) {
    const dependency = dependenciesForChangeSet(item, ordered);
  ...
  const phaseMap = Object.fromEntries(phases.map((phase) => [phase, [] as ChangeSet[]])) as Record<ChangeSetReplayPhase, ChangeSet[]>;
  for (const item of ordered) phaseMap[dependenciesForChangeSet(item).phase].push(item);
  const phaseOrdered = phases.flatMap((phase) => phaseMap[phase]);
  return { ordered: phaseOrdered, phases: phaseMap, blockers, digest: await digestChangeSetBatch(phaseOrdered) };
}

export const buildChangeSetQueuePlan = planChangeSetQueue;

export async function createClaimedBatch(claimId: string, changeSets: readonly ChangeSet[]): Promise<ClaimedBatch> {
  assertNonEmptyString(claimId, "claimId");
  const ordered = [...changeSets].sort(compareChangeSets);
  return { claimId, changeSetIds: ordered.map((item) => item.id), digest: await digestChangeSetBatch(ordered) };
}

export async function verifyClaimedBatchDigest(claim: ClaimedBatch, changeSets: readonly ChangeSet[]): Promise<boolean> {
  if (!isRecord(claim) || typeof claim.claimId !== "string" || !Array.isArray(claim.changeSetIds) || !CHANGE_SET_DIGEST_PATTERN.test(claim.digest)) return false;
  ...
  return (await digestChangeSetBatch(selected as ChangeSet[])) === claim.digest;
}

export async function assertClaimedBatchDigest(claim: ClaimedBatch, changeSets: readonly ChangeSet[]): Promise<void> {
  if (!await verifyClaimedBatchDigest(claim, changeSets)) throw new Error("change-set claim acknowledgement digest mismatch");
}

export const checkClaimedBatchDigest = verifyClaimedBatchDigest;
