/**
 * Event reducer and batch replay for the canonical projection.  The reducer
...
import {
  updateBankDeleteCascade,
  updateQuestionDeleteCascade,
  updateQuestionsBulkDeleteCascade,
} from "./change-set-cascade";
import {
  projectionValidationIssues,
  recomputeProjectionInPlace,
} from "./change-set-derived";

function applyMutation(projection: ChangeSetProjection, mutation: ChangeSetMutation, context: { createdAt: string; deviceId: string; eventId: string; localSequence: number }): void {
  switch (mutation.kind) {
    ...
    case "bank.delete": case "bank.delete.cascade": {
      ...
      updateBankDeleteCascade(projection, bank.id, deletedAt, context.deviceId, context.eventId, context.localSequence);
      return;
    }
 ...
 *  once via finalizeRebasedProjection instead of per record. */
export function applyChangeSetToOwnedProjection(projection: ChangeSetProjection, changeSet: ChangeSet): ChangeSetProjection {
...
export function finalizeRebasedProjection(projection: ChangeSetProjection): ChangeSetProjection {
  const rebuilt = recomputeProjectionInPlace(projection);
  const issues = projectionValidationIssues(rebuilt, { verifyDerived: false });
...
export function reduceChangeSet(input: ChangeSetProjectionInput, changeSet: ChangeSet): ChangeSetProjection {
  return finalizeRebasedProjection(applyChangeSetToOwnedProjection(normalizeProjection(input), changeSet));
}
...
export function replayChangeSetBatch(input: ChangeSetProjectionInput, changes: readonly ChangeSet[], onStep?: (done: number, total: number) => void, options?: { onConflict?: "skip" | "throw" }): { projection: ChangeSetProjection; skipped: string[] } {
  ...
  for (let index = 0; index < changes.length; index += 1) {
    ...
    try {
      good = applyChangeSetToOwnedProjection(good, change);
    } catch (error) {
      if (!skip) throw error;
  ...
  return { projection: finalizeRebasedProjection(good), skipped };
}

export const applyChangeSet = reduceChangeSet;

export function reduceChangeSets(input: ChangeSetProjectionInput, changeSets: readonly ChangeSet[]): ChangeSetProjection {
  ...
  for (const changeSet of ordered) projection = reduceChangeSet(projection, changeSet);
  return projection;
}

export const replayChangeSets = reduceChangeSets;
