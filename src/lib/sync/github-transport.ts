...
export interface GitHubTreeMutation {
  path: string;
  blobSha: string | null;
}
...
function repoGitPath(client: GitHubRequestClient, suffix: string): string {
...
export async function readGitHubContentsAtRef(client: GitHubRequestClient, path: string, ref: string): Promise<Uint8Array | null> {
...
export async function createGitHubBlob(client: GitHubRequestClient, bytes: Uint8Array): Promise<string> {
...
export async function readGitHubBranchSnapshot(client: GitHubRequestClient): Promise<GitHubBranchSnapshot> {
...
export async function commitGitHubTreeFastForward(
  ...
  base: GitHubBranchSnapshot,
  mutations: readonly GitHubTreeMutation[],
  message: string,
): Promise<boolean> {
