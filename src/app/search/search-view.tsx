...
export function SearchView({
  ...
  const derivedSearchData = useMemo(() => buildSearchDerivedData({
    ...
    attemptStats: historyData?.attemptStats ?? [],
    attempts: historyData?.attempts ?? [],
    notes: historyData?.notes ?? [],
    roundProgress: historyData?.roundProgress ?? [],
    progressScope: normalizedSearchScope,
    referenceTime,
    wrongRemovalStreak,
  }), [appliedQuestions, historyData, normalizedSearchScope, referenceTime, wrongRemovalStreak]);
  ...
  const searchDataReady = showResults && views !== undefined && historyData !== undefined && historyData !== null;
  ...
  useEffect(() => {
    ...
    if (!searchDataReady || !searchWorkerClient) {
  ...
  }, [debouncedQuery, derivedSearchData.index, filterProjection, searchClientRef, searchDataReady, searchIndexKey, searchRequestKey, typeTab]);

  const searchPending = showResults && (!searchDataReady || completedSearch?.key !== searchRequestKey);
  ...
  function updateFilters(nextFilters: SearchFilters) {
    ...
    setTypeTab(nextFilters.questionType === "all" ? "全部" : nextFilters.questionType);
    setVisibleCount(50);
    setSelectedIds([]);
  }

  const totalCount = TYPE_ORDER.reduce((sum, type) => sum + result.counts[type], 0);
  ...
  const filterChips = [
    ...
    filters.questionType !== "all" ? `题型：${filters.questionType}` : "",
