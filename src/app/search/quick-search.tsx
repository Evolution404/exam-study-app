...
export function QuickSearch({ banks, activeBankIds, onOpenSearch }: {
  ...
  return <div ref={boxRef} className={`searchbox ${open && draft.trim() ? "results-open" : ""}`}>
    ...
    <input aria-label="快速正则搜索题目、答案、选项、标签或解析" value={draft} onFocus={() => { setOpen(true); }} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") { setDraft(""); setOpen(false); } else if (event.key === "Enter") { event.currentTarget.blur(); openSearch(); } }} placeholder="快速搜索" />
