...
import { deleteQuestions } from "@/lib/db/db";
import type { Question } from "@/lib/db/types";
import { loadImageAsset, SharedQuestionEditor, toQuestionViewModel } from "@/app/bank/question-editor";
import { ContentBlockRenderer } from "@/app/bank/content-block-renderer";
import { ConfirmDialog } from "@/app/ui/confirm-dialog";

export function UnfiledQuestionSection({ questions, onNotice }: { questions: Question[]; onNotice: (message: string) => void }) {
  ...
  const allSelected = models.length > 0 && models.every((question) => selectedIdSet.has(question.id));

  async function deleteSelected() {
    try {
      setDeleting(true);
      const count = await deleteQuestions(selectedIds);
      setSelectedIds([]);
