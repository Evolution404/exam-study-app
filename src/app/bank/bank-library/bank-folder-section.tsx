...
import { ArrowDown, ArrowUp, BookOpenCheck, ChevronDown, ChevronRight, Folder, FolderOpen, GripVertical, Library, Pencil, Trash2 } from "lucide-react";
...
const UNFILED_COLLAPSE_STORAGE_ID = "__unfiled__";

function readFolderCollapsed(storageId: string) {
  if (typeof window === "undefined") return false;
  try {
    return window.localStorage.getItem(`${FOLDER_COLLAPSE_STORAGE_PREFIX}${storageId}`) === "1";
  } catch {
    return false;
  }
}

function writeFolderCollapsed(storageId: string, collapsed: boolean) {
  try {
    const key = `${FOLDER_COLLAPSE_STORAGE_PREFIX}${storageId}`;
...
function SortableBankItem({ bank, index, total, onOpen, onMove, onToggleEnabled, reorderEnabled }: { bank: Bank; index: number; total: number; onOpen: (bank: Bank) => void; onMove: (bank: Bank, offset: number) => void; onToggleEnabled: (bank: Bank) => void; reorderEnabled: boolean }) {
  ...
  return <article ref={setNodeRef} data-drag-id={bank.id} data-drag-index={index} data-enabled={enabled} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.6 : undefined }} className={isDragging ? "drag-active" : ""}>
    <button type="button" className="bank-drag" disabled={!reorderEnabled} aria-label={reorderEnabled ? `拖动${title}排序` : `${title}筛选视图不可排序`} style={{ opacity: reorderEnabled ? 1 : .28, cursor: reorderEnabled ? "grab" : "default" }} {...attributes} {...listeners}><GripVertical size={18} /></button>
    <button className="bank-management-main" onClick={() => onOpen(bank)}><span className="bank-color" style={{ background: bank.color || "#dfe9e2", opacity: enabled ? 1 : .55 }}><BookOpenCheck size={18} /></span><span><strong>{title}</strong><small>{bank.questionCount.toLocaleString()} 题 · {fullDate(bank.importedAt)}{enabled ? "" : " · 已停用"}</small></span><ChevronRight size={17} /></button>
    <div className="bank-row-actions">
      <button type="button" className="bank-enable-toggle" role="switch" aria-checked={enabled} aria-label={`${enabled ? "停用" : "启用"}题库${title}`} onClick={() => onToggleEnabled(bank)}><span className="bank-enable-toggle-track" aria-hidden="true"><span /></span></button>
      {reorderEnabled && <div className="bank-order-buttons"><button aria-label="向上移动" disabled={index === 0} onClick={() => onMove(bank, -1)}><ArrowUp size={14} /></button><button aria-label="向下移动" disabled={index === total - 1} onClick={() => onMove(bank, 1)}><ArrowDown size={14} /></button></div>}
    </div>
  </article>;
}
...
export function BankFolderSection({ folder, banks, draggedBankId, onDrag, onDrop, onOpen, onMove, onToggleEnabled, reorderEnabled = true, onEditFolder, onDeleteFolder }: { folder?: BankFolder; banks: Bank[]; draggedBankId?: string; onDrag: (id?: string) => void; onDrop: (beforeId?: string) => void; onOpen: (bank: Bank) => void; onMove: (bank: Bank, offset: number) => void; onToggleEnabled: (bank: Bank) => void; reorderEnabled?: boolean; onEditFolder?: () => void; onDeleteFolder?: () => void }) {
  ...
  const collapseStorageId = folder?.id ?? UNFILED_COLLAPSE_STORAGE_ID;
  const [collapsed, setCollapsed] = useState(() => readFolderCollapsed(collapseStorageId));
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const previewChanged = banks.some((bank, currentIndex) => ordered[currentIndex]?.id !== bank.id);
  ...
  function toggleCollapsed() {
    setCollapsed((value) => {
      const next = !value;
      writeFolderCollapsed(collapseStorageId, next);
      return next;
    });
  }

  const collapseLabel = folder ? `${collapsed ? "展开" : "折叠"}文件夹${folder.name}` : `${collapsed ? "展开" : "折叠"}未分组题库`;
  const folderMeta = `${banks.length} 个题库${folder?.description ? ` · ${folder.description}` : ""}`;

  return <section className={`bank-folder ${draggedBankId ? "drag-active" : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); onDrop(); }}><header style={{ padding: 0, display: "flex", gap: 0, borderBottom: collapsed ? 0 : undefined }}><button type="button" className="result-group-toggle" style={{ width: "auto", minWidth: 0, flex: 1, padding: "16px 10px 16px 18px" }} aria-expanded={!collapsed} aria-label={collapseLabel} onClick={toggleCollapsed}><span className="folder-icon">{folder ? (collapsed ? <Folder size={18} /> : <FolderOpen size={18} />) : <Library size={18} />}</span><span className="result-group-title">{folder?.name ?? "未分组"}</span><span className="result-group-count" style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{folderMeta}</span><ChevronDown size={16} style={{ transform: collapsed ? "rotate(-90deg)" : "none" }} /></button>{folder && <div className="folder-actions" style={{ paddingRight: 18 }}><button aria-label={`编辑文件夹${folder.name}`} onClick={onEditFolder}><Pencil size={15} /></button><button aria-label={`删除文件夹${folder.name}`} onClick={onDeleteFolder}><Trash2 size={15} /></button></div>}</header>{!collapsed && <><DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}><SortableContext items={ordered.map((bank) => bank.id)} strategy={verticalListSortingStrategy}><div className="bank-management-grid">{ordered.map((bank, currentIndex) => <SortableBankItem key={bank.id} bank={bank} index={currentIndex} total={ordered.length} onOpen={onOpen} onMove={onMove} onToggleEnabled={onToggleEnabled} reorderEnabled={reorderEnabled} />)}</div></SortableContext></DndContext>{!ordered.length && <div className="folder-drop-empty"><Folder size={20} />将题库拖到这里</div>}</>}</section>;
}
