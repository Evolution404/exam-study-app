...
import { Check, ChevronLeft, ChevronRight, CircleAlert, ClipboardCheck, Copy, X } from "lucide-react";
...
export function QuestionDetail({ question, metric, scopeLabel, note, onClose, footer, nav, answer, answerLabel = "你的答案" }: {
  ...
  // 详情页复制题目：只复制题目+选项，不含答案；做错的题附上「我的选择」
  // （原始字母，与本页正文按 canonical 顺序渲染一致）。answer 缺省的调用方
  // （搜索/题库管理/知识页）自然退化为纯题目复制。
  async function handleCopyQuestion() {
    const wrongSelection = answer?.submitted && answer.correct === false ? answer.selected : undefined;
    const ok = await copyTextToClipboard(buildQuestionCopyText(question, { wrongSelection }));
