...
import { SharedQuestionEditor, loadImageAsset } from "@/app/bank/question-editor";
...
import { displayedAnswer, playAnswerFeedback, recordPracticeAnswer, saveNote, summarizeAttemptStats } from "../helpers";
...
export function Practice({ runId, question, initialState, optionOrder, questionIds, questionTypes, answers, index, total, modeLabel, preferences, transitionPending = false, onStateChange, onJump, onFavorite, onPrevious, onNext, onFinish, onExit }: PracticeProps) {
  ...
  const note = useLiveQuery(() => studyDb.notes.get(question.id), [question.id]);
  const attemptSummary = useLiveQuery(async () => summarizeAttemptStats(await studyDb.questionProgress.get(question.id)), [question.id]) ?? summarizeAttemptStats();
  ...
  async function handleCopyQuestion(target: "question" | "questionWithAnswer") {
    ...
    const text = buildQuestionCopyText(question, { displayOrder, includeAnswer: target === "questionWithAnswer", wrongSelection });
    ...
    <div className="practice-layout">
      <section ref={questionCardRef} className="question-card" data-question-id={question.id} data-question-index={index} data-transition-pending={transitionPending ? "true" : "false"} aria-busy={transitionPending || undefined} inert={transitionPending || undefined}>
        ...
        <div className="question-body">
          ...
          />
          {question.type === "计算" && hasInlineCalculationBlanks ? <CalculationContentRenderer blocks={question.canonical.content} loadAsset={loadImageAsset} className="practice-stem calculation-practice-stem" answerCount={expectedCalculationAnswers.length} values={submitted ? selected : calculationDrafts} expected={expectedCalculationAnswers} tolerancePercent={preferences.calculationTolerancePercent} disabled={submitted} idPrefix={`calculation-answer-${question.id}`} onChange={(blankIndex, value) => setCalculationDrafts((current) => current.map((item, itemIndex) => itemIndex === blankIndex ? value : item))} onLastEnter={() => void submit()} /> : question.type === "填空" && hasInlineFillBlanks ? <FillContentRenderer blocks={question.canonical.content} loadAsset={loadImageAsset} className="practice-stem fill-practice-stem" blankCount={expectedFillSolution?.blanks.length ?? 0} values={submitted ? selected : fillDrafts} expected={expectedFillSolution} disabled={submitted} idPrefix={`fill-answer-${question.id}`} onChange={(blankIndex, value) => setFillDrafts((current) => current.map((item, itemIndex) => itemIndex === blankIndex ? value : item))} onLastEnter={() => void submit()} /> : <ContentBlockRenderer blocks={question.canonical.content} loadAsset={loadImageAsset} className="practice-stem" />}
