...
import { areCalculationAnswersCorrect, calculationAnswers, calculationBlankIndexes, formatCalculationAnswers } from "@/lib/question/question-utils";
...
export function Practice({ runId, question, initialState, optionOrder, questionIds, questionTypes, answers, index, total, modeLabel, preferences, onStateChange, onJump, onFavorite, onPrevious, onNext, onFinish, onExit }: { runId: string; question: Question; initialState?: PracticeAnswerState; optionOrder?: number[]; questionIds: string[]; questionTypes: Record<string, QuestionType>; answers: Record<string, PracticeAnswerState>; index: number; total: number; modeLabel: string; preferences: PracticePreferences; onStateChange: (state: PracticeAnswerState) => void; onJump: (index: number) => void; onFavorite: () => Promise<void>; onPrevious: () => void; onNext: () => void; onFinish: () => void; onExit: () => void }) {
  ...
  const expectedCalculationAnswers = question.type === "计算" ? calculationAnswers(question.answer) : [];
  ...
  const selectedCanonical = question.type === "计算" ? selected.join("\n") : [...selected].sort().join("");
  const selectedAnswer = question.type === "计算" ? formatCalculationAnswers(selected) : selected
  ...
  async function submit(valueList = selected) {
    ...
    if (!value || (question.type === "计算" && !calculationInputValid) || submitted || answering.current) return;
    answering.current = true;
    const finalSelection = question.type === "计算" ? calculationValues : valueList;
    ...
    try {
      const result = await recordPracticeAnswer({ runId, questionId: question.id, bankId: question.bankId, selected: finalSelection, correct: isCorrect, elapsedMs: activeTimer.current?.elapsedMs(window.performance.now()) ?? 0 });
